...
class LiquidityManagementService:
    ...
    def __init__(self, redis_client: redis.Redis, db_session: Session):
        ...
        # Historical data
        self.liquidity_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=10000))
    ...
    async def get_liquidity_pools(self, symbol: Optional[str] = None) -> List[LiquidityPool]:
        """Get liquidity pools"""
        try:
    ...
    async def create_liquidity_pool(self, symbol: str, pool_type: str, 
                                  total_liquidity: float, providers: List[str],
                                  fees: Dict[str, float]) -> LiquidityPool:
    ...
    async def allocate_liquidity(self, symbol: str, user_id: int, allocation_type: str,
                               allocated_amount: float) -> LiquidityAllocation:
    ...
    async def get_liquidity_optimization(self, symbol: str) -> Optional[LiquidityOptimization]:
    ...
    async def get_liquidity_alerts(self, symbol: Optional[str] = None, 
                                 severity: Optional[str] = None) -> List[LiquidityAlert]:
    ...
    async def calculate_liquidity_score(self, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            spread_score = max(0, 1 - profile.bid_ask_spread / 0.1)  # Lower spread = higher score
            volume_score = min(profile.volume_profile.get('avg_volume', 0) / 100000, 1.0)
            ...
            confidence = min(
                len(self.liquidity_history.get(symbol, [])) / 1000,
                1.0
            )
    ...
    async def optimize_liquidity_allocation(self, symbol: str, 
                                          total_amount: float) -> Dict[str, Any]:
        """Optimize liquidity allocation across pools"""
        try:
            pools = [p for p in self.liquidity_pools.values() if p.symbol == symbol and p.is_active]
            ...
            portfolio_variance = np.dot(optimal_weights, np.dot(correlation_matrix, optimal_weights))
        ...
        try:
            ...
            if profile.liquidity_score < 0.3:
                await self._create_liquidity_alert(
                    symbol, LiquidityEvent.LIQUIDITY_CRISIS, 'high',
            ...
            if profile.bid_ask_spread > 0.01:
                await self._create_liquidity_alert(
                    symbol, LiquidityEvent.SPREAD_WIDENING, 'medium',
                    f"Spread widening detected for {symbol}",
                    0.01, profile.bid_ask_spread
                )
            
            # Check for volume surge
            if profile.volume_profile.get('avg_volume', 0) > 300000:
                await self._create_liquidity_alert(
                    symbol, LiquidityEvent.VOLUME_SURGE, 'medium',
                    f"Volume surge detected for {symbol}",
                    300000, profile.volume_profile.get('avg_volume', 0)
                )
            
            # Check for market stress
            if profile.volatility > 0.4:
                await self._create_liquidity_alert(
                    symbol, LiquidityEvent.MARKET_STRESS, 'high',
                    f"Market stress detected for {symbol}",
                    0.4, profile.volatility
                )
            
        except Exception as e:
            logger.error(f"Error checking liquidity events: {e}")
    
    async def _create_liquidity_alert(self, symbol: str, event_type: LiquidityEvent,
                                    severity: str, message: str, threshold: float,
                                    current_value: float):
    ...
    async def _optimize_symbol_liquidity_allocation(self, symbol: str):
        ...
        try:
            ...
            allocations = [a for a in self.liquidity_allocations.values() if a.symbol == symbol]
            ...
            optimization_result = await self.optimize_liquidity_allocation(symbol, total_amount)
    ...
    async def _generate_recommendations(self, optimization_result: Dict[str, Any]) -> List[str]:
        """Generate optimization recommendations"""
        try:
            recommendations = []
            
            if optimization_result['sharpe_ratio'] < 0.5:
                recommendations.append("Consider rebalancing portfolio for better risk-adjusted returns")
            
            if optimization_result['diversification_ratio'] < 0.5:
                recommendations.append("Increase diversification across liquidity pools")
            
            if optimization_result['portfolio_risk'] > 0.3:
                recommendations.append("Reduce portfolio risk through conservative allocations")
    ...
    async def _generate_liquidity_alerts(self):
        ...
        while True:
            try:
                ...
                total_liquidity = sum(p.total_liquidity for p in self.liquidity_pools.values())
                total_utilized = sum(p.utilized_liquidity for p in self.liquidity_pools.values())
                system_utilization = total_utilized / total_liquidity if total_liquidity > 0 else 0
                
                if system_utilization > 0.9:
                    await self._create_system_alert(
                        "System-wide liquidity utilization high",
                        "critical", system_utilization
                    )
    ...
    async def _update_allocation_performance(self, allocation: LiquidityAllocation):
        ...
        try:
            ...
            allocation.utilized_amount = allocation.allocated_amount * allocation.utilization_rate
            ...
            for i, symbol1 in enumerate(symbols):
                ...
                for j, symbol2 in enumerate(symbols):
                    if i == j:
                        ...
                        self.correlation_matrix[symbol1][symbol2] = np.random.uniform(0.3, 0.8)
...
# Factory function
async def get_liquidity_management_service(redis_client: redis.Redis, db_session: Session) -> LiquidityManagementService:
