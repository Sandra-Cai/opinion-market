...
class RealTimeAnalyticsService:
    ...
    def __init__(self, redis_client: redis.Redis, db_session: Session):
        ...
        self.system_metrics = SystemMetrics(
            total_users=0, active_users=0, total_markets=0, active_markets=0,
            total_volume_24h=0.0, total_trades_24h=0, avg_response_time=0.0,
            error_rate=0.0, cpu_usage=0.0, memory_usage=0.0,
            database_connections=0, cache_hit_rate=0.0, last_updated=datetime.utcnow()
        )
    ...
    async def _calculate_market_metrics(self, market_id: int):
        ...
        try:
            ...
            price_change_24h = ((current_price - price_24h_ago) / price_24h_ago * 100) if price_24h_ago else 0
    ...
    async def get_market_trends(self, market_id: int, hours: int = 24) -> Dict[str, List[float]]:
        ...
        try:
            ...
            price_data = await self._get_historical_prices(market_id, start_time, end_time)
            volume_data = await self._get_historical_volumes(market_id, start_time, end_time)
    ...
    async def _cache_market_metrics(self):
        ...
        try:
            for market_id, metrics in self.market_data_cache.items():
                ...
                await self.redis.setex(cache_key, 300, json.dumps(cache_data))  # 5 minutes TTL
        except Exception as e:
    ...
    async def _cache_user_metrics(self):
        ...
        try:
            for user_id, metrics in self.user_data_cache.items():
                ...
                await self.redis.setex(cache_key, 600, json.dumps(cache_data))  # 10 minutes TTL
        except Exception as e:
    ...
    async def _cache_system_metrics(self):
        ...
        try:
            ...
            await self.redis.setex(cache_key, 60, json.dumps(cache_data))  # 1 minute TTL
        except Exception as e:
    ...
    async def _get_historical_prices(self, market_id: int, start_time: datetime, end_time: datetime) -> List[Dict]:
    ...
    async def _get_historical_volumes(self, market_id: int, start_time: datetime, end_time: datetime) -> List[Dict]:
...
# Factory function
async def get_real_time_analytics_service(redis_client: redis.Redis, db_session: Session) -> RealTimeAnalyticsService:
