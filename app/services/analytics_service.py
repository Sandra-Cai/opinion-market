...
class AnalyticsService:
    ...
    def get_market_analytics(self, market_id: int) -> Dict:
        ...
        if len(trades) > 1:
            ...
            price_change = ((last_trade.price_per_share - first_trade.price_per_share) / first_trade.price_per_share) * 100
        else:
            price_change = 0
        ...
        # Trader sentiment
        buy_volume = sum(trade.total_value for trade in trades if trade.trade_type == 'buy')
        sell_volume = sum(trade.total_value for trade in trades if trade.trade_type == 'sell')
    ...
    def get_user_analytics(self, user_id: int) -> Dict:
        ...
        portfolio_return = ((portfolio_value - total_invested) / total_invested * 100) if total_invested > 0 else 0
    ...
    def get_platform_analytics(self) -> Dict:
        ...
        resolved_markets = self.db.query(Market).filter(Market.status == 'resolved').count()
    ...
    def get_market_predictions(self, market_id: int) -> Dict:
        ...
        times = [(trade.created_at - recent_trades[0].created_at).total_seconds() for trade in recent_trades]
        
        # Calculate trend
        if len(prices) > 1:
            price_trend = (prices[-1] - prices[0]) / (times[-1] - times[0]) if times[-1] > times[0] else 0
    ...
    def _get_volume_by_time(self, market_id: int, time_unit: str) -> List[Dict]:
        ...
        if time_unit == 'hour':
            ...
            ).filter(Trade.market_id == market_id).group_by(
                func.date_trunc('hour', Trade.created_at)
            ).order_by('time').all()
        else:  # day
            trades = self.db.query(
                ...
                func.date_trunc('day', Trade.created_at)
            ).order_by('time').all()
        
        return [{"time": str(trade.time), "volume": float(trade.volume)} for trade in trades]
    ...
    def _get_volume_by_category(self) -> List[Dict]:
        ...
        result = self.db.query(
            Market.category,
            func.sum(Trade.total_value).label('volume')
        ).join(Trade, Market.id == Trade.market_id).group_by(
            Market.category
        ).order_by(desc('volume')).all()
        
        return [{"category": str(r.category), "volume": float(r.volume)} for r in result]
    
    def _get_trending_markets(self, limit: int = 10) -> List[Dict]:
        """Get trending markets based on recent activity"""
        trending = self.db.query(Market).filter(
            Market.status == 'open'
        ).order_by(desc(Market.trending_score)).limit(limit).all()
    ...
    def _get_user_market_preferences(self, user_id: int) -> List[Dict]:
        ...
        ).join(Trade, Market.id == Trade.market_id).filter(
            Trade.user_id == user_id
        ).group_by(Market.category).order_by(desc('total_volume')).all()
