...
def get_top_traders(
    ...
    if since:
        ...
        recent_traders = db.query(Trade.user_id).filter(
...
def get_top_market_creators(
    ...
    ).join(Market, User.id == Market.creator_id).filter(
        User.is_active == True
...
def get_top_win_rate_traders(
    ...
    top_traders = db.query(User).filter(
        User.is_active == True,
        User.total_trades >= min_trades
...
def get_top_reputation_traders(
    ...
    ).order_by(desc(User.reputation_score)).limit(limit).all()
