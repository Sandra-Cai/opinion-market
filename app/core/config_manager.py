...
class ConfigManager:
    ...
    def _load_config(self):
        ...
        logger.info(
            f"Configuration loaded for environment: {self.config.environment.value}"
        )
    ...
    def validate_config(self) -> Dict[str, Any]:
        ...
        if (
            ...
            validation_results["warnings"].append(
                "Using default secret key in production is not secure"
            )
        ...
        if self.config.database.pool_size < 1 or self.config.database.pool_size > 100:
            validation_results["errors"].append(
                "Database pool size should be between 1 and 100"
            )
            validation_results["valid"] = False
