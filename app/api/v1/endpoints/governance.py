...
def create_proposal(
    ...
    user_tokens = (
        db.query(GovernanceToken)
        .filter(GovernanceToken.user_id == current_user.id)
        .first()
    )

    if (
        not user_tokens or user_tokens.total_voting_power < 1000
...
def get_proposals(
    ...
    total = query.count()
    proposals = (
        query.order_by(desc(GovernanceProposal.created_at))
        .offset(skip)
        .limit(limit)
        .all()
    )
    ...
    return GovernanceProposalListResponse(
        proposals=proposals, total=total, page=skip // limit + 1, per_page=limit
    )
...
def get_proposal(proposal_id: int, db: Session = Depends(get_db)):
    ...
    proposal = (
        ...
        .filter(GovernanceProposal.id == proposal_id)
        .first()
    )
    if not proposal:
        raise HTTPException(
...
def vote_on_proposal(
    ...
    if not user_tokens or user_tokens.available_amount < vote_data.voting_power:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Insufficient voting power"
        )
...
def get_proposal_votes(
    ...
    votes = (
        ...
        .order_by(desc(GovernanceVote.created_at))
        .offset(skip)
        .limit(limit)
        .all()
    )

    return {"proposal_id": proposal_id, "votes": votes, "total": len(votes)}
...
def get_my_tokens(
    ...
    if not tokens:
        ...
        tokens = GovernanceToken(
            user_id=current_user.id, token_amount=100.0  # Default tokens for new users
        )
        db.add(tokens)
        db.commit()
        db.refresh(tokens)
...
def stake_tokens(
    ...
    if not tokens:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No governance tokens found"
        )
...
def unstake_tokens(
    ...
    if tokens.staked_amount < unstake_data.amount:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Insufficient staked tokens"
        )
...
def get_governance_stats(db: Session = Depends(get_db)):
    ...
    active_proposals = (
        db.query(GovernanceProposal)
        .filter(GovernanceProposal.status == ProposalStatus.ACTIVE)
        .count()
    )
    passed_proposals = (
        db.query(GovernanceProposal)
        .filter(GovernanceProposal.status == ProposalStatus.PASSED)
        .count()
    )
    rejected_proposals = (
        db.query(GovernanceProposal).filter(ProposalStatus.REJECTED).count()
    )
    ...
    top_voters = (
        ...
        .group_by(GovernanceVote.voter_id)
