...
class MarketDataAnalyticsService:
    ...
    def __init__(self, redis_client: redis.Redis, db_session: Session):
        ...
        self.analytics_performance: Dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))
    ...
    async def add_market_data(self, market_id: str, data_point: MarketDataPoint) -> bool:
    ...
    async def get_market_data(self, market_id: str, timeframe: Timeframe = Timeframe.DAY,
                             start_time: Optional[datetime] = None, end_time: Optional[datetime] = None,
                             limit: int = 1000) -> List[MarketDataPoint]:
    ...
    async def calculate_technical_indicators(self, market_id: str) -> Dict[str, MarketIndicator]:
    ...
    async def create_market_alert(self, market_id: str, alert_type: str, condition: str,
                                 threshold: float, severity: str = 'medium') -> MarketAlert:
    ...
    async def get_market_intelligence(self, market_id: str, report_type: str = 'daily',
                                    limit: int = 10) -> List[MarketIntelligence]:
        ...
        try:
            ...
            if report_type != 'all':
                intelligence_list = [i for i in intelligence_list if i.report_type == report_type]
    ...
    async def get_data_quality(self, market_id: str) -> DataQuality:
        ...
        try:
            return self.data_quality.get(market_id, DataQuality(
                market_id=market_id,
                completeness=0.0,
                accuracy=0.0,
                timeliness=0.0,
    ...
    async def _update_data_quality(self, market_id: str, data_point: MarketDataPoint):
        ...
        try:
            ...
            if len(data_points) > 0:
                quality.completeness = min(1.0, len(data_points) / 1000)  # Normalize to 1000 points
            
            # Update accuracy (data consistency)
            if len(data_points) >= 2:
                last_point = data_points[-2]
                price_change = abs(data_point.close_price - last_point.close_price) / last_point.close_price
            ...
            quality.reliability = (quality.timeliness + quality.completeness + quality.accuracy + quality.consistency) / 4
    ...
    async def _process_real_time_data(self, market_id: str, data_point: MarketDataPoint):
            ...
            for alert in self.alerts[market_id]:
                ...
                if alert.alert_type == 'price':
                    ...
                    if len(data_points) >= 20:
                        recent_prices = [dp.close_price for dp in list(data_points)[-20:]]
                ...
                for indicator_name, indicator in self.indicators[market_id].items():
                    if indicator_name == 'price':
                        ...
                        if indicator.previous_value > 0:
                            indicator.change_percent = (indicator.change / indicator.previous_value) * 100
    ...
    async def _update_performance_metrics(self, market_id: str, data_point: MarketDataPoint):
        """Update performance metrics"""
        try:
            # Update data latency
    ...
    async def _calculate_bollinger_bands(self, df: pd.DataFrame, period: int, std_dev: float) -> Dict[str, Optional[float]]:
        """Calculate Bollinger Bands"""
        try:
            if len(df) < period:
    ...
    async def _calculate_stochastic(self, df: pd.DataFrame, k_period: int, d_period: int) -> Dict[str, Optional[float]]:
                ...
                for market_id, alert_list in self.alerts.items():
                    for alert in alert_list:
                        if alert.status == 'triggered':
                            # Log triggered alert
                            logger.info(f"Alert triggered: {alert.alert_id} for market {market_id}")
    ...
    async def _cache_market_data(self, data_point: MarketDataPoint):
        ...
        try:
            cache_key = f"market_data:{data_point.market_id}:{data_point.timestamp.isoformat()}"
...
# Factory function
async def get_market_data_analytics_service(redis_client: redis.Redis, db_session: Session) -> MarketDataAnalyticsService:
