...
class SystemMonitor:
        ...
        while True:
            try:
                ...
                self.application_metrics_history = [
                    m for m in self.application_metrics_history 
                    if m.timestamp > cutoff_time
                ]
    ...
    async def _store_metrics_in_redis(self, system_metrics: SystemMetrics, app_metrics: ApplicationMetrics):
        """Store metrics in Redis for external monitoring"""
        try:
            metrics_data = {
                'system': {
    ...
    async def _analyze_performance_trends(self):
        ...
        try:
            ...
            response_time_trend = (response_times[-1] - response_times[0]) / len(response_times)
            ...
            if error_rate_trend > 1:  # Error rate increasing by more than 1% per interval
                await self._create_alert(
    ...
    async def get_application_metrics(self, hours: int = 24) -> List[ApplicationMetrics]:
        """Get application metrics for the specified time period"""
        cutoff_time = datetime.utcnow() - timedelta(hours=hours)
        return [m for m in self.application_metrics_history if m.timestamp > cutoff_time]
