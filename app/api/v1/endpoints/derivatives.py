...
from app.services.derivatives_trading import get_derivatives_trading_service, DerivativesTradingService
from app.services.derivatives_risk_management import get_derivatives_risk_management_service, DerivativesRiskManagementService
from app.schemas.derivatives import (
    DerivativeCreate, DerivativeResponse, DerivativePriceResponse,
    DerivativePositionCreate, DerivativePositionResponse,
    DerivativeOrderCreate, DerivativeOrderResponse,
    OptionPriceRequest, OptionPriceResponse,
    VolatilitySurfaceResponse, GreeksResponse,
    RiskLimitCreate, RiskLimitResponse,
    RiskMetricResponse, StressTestRequest, StressTestResponse,
    RiskReportResponse, PortfolioRiskResponse
)
...
async def create_derivative(
    ...
    try:
        ...
        return DerivativeResponse(
            ...
            option_type=derivative.option_type.value if derivative.option_type else None,
            exercise_style=derivative.exercise_style.value if derivative.exercise_style else None,
...
async def list_derivatives(
    ...
    try:
        ...
        # Apply filters
        if derivative_type:
            derivatives = [d for d in derivatives if d.derivative_type.value == derivative_type]
        if underlying_asset:
            derivatives = [d for d in derivatives if d.underlying_asset == underlying_asset]
...
@router.get("/derivatives/{derivative_id}/price", response_model=DerivativePriceResponse)
...
@router.get("/volatility-surfaces/{underlying_asset}", response_model=VolatilitySurfaceResponse)
async def get_volatility_surface(
    ...
    try:
        volatility_surface = await derivatives_service.get_volatility_surface(underlying_asset)
...
@router.get("/users/{user_id}/positions", response_model=List[DerivativePositionResponse])
...
async def get_derivatives_summary(
    ...
    try:
        ...
        active_derivatives = len([d for d in derivatives_service.derivatives.values() if d.is_active])
...
async def get_risk_summary(
    ...
    try:
        ...
        active_limits = sum(len([l for l in limits if l.is_active]) for limits in risk_service.risk_limits.values())
        
        # Count breaches
        total_breaches = sum(sum(l.breach_count for l in limits) for limits in risk_service.risk_limits.values())
