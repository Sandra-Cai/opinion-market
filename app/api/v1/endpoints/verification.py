...
def get_verification_stats(
    ...
    pending_count = (
        db.query(Market)
        .filter(Market.status == MarketStatus.PENDING_VERIFICATION)
        .count()
    )

    verified_today = (
        db.query(Market).filter(Market.verified_at >= datetime.utcnow().date()).count()
    )
    ...
    total_verified = db.query(Market).filter(Market.verified_by.isnot(None)).count()
...
def reactivate_market(
    ...
    if market.status != MarketStatus.SUSPENDED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Market is not suspended"
        )
