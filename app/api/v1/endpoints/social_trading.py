...
from app.schemas.social_trading import (
    TraderProfileRequest, TraderProfileResponse,
    SocialSignalRequest, SocialSignalResponse,
    CopyTradeRequest, CopyTradeResponse,
    LeaderboardResponse, CommunityPostRequest, CommunityPostResponse,
    TraderAnalyticsResponse, TraderFeedResponse
)
...
async def follow_trader(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=400, detail="Already following this trader or invalid request")
...
async def get_social_trading_stats(
    ...
    try:
        ...
        stats = {
            ...
            "active_signals": len([s for s in social_service.social_signals.values() if s.expires_at > datetime.utcnow()]),
            "active_copy_trades": len([c for c in social_service.copy_trades.values() if c.status == 'active']),
            "top_performers": [
                {
                    "user_id": profile.user_id,
                    "username": profile.username,
        ...
        raise HTTPException(status_code=500, detail="Error getting social trading stats")
