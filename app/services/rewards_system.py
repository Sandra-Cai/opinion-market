...
class RewardsSystem:
    ...
    def check_daily_login_reward(self, user_id: int) -> Dict:
        ...
        try:
            ...
            governance_tokens = (
                ...
                .filter(GovernanceToken.user_id == user_id)
                .first()
            )
            ...
            if governance_tokens:
                ...
                governance_tokens = GovernanceToken(
                    user_id=user_id, token_amount=total_tokens
                )
    ...
    def _award_first_trade_reward(self, user: User, db: Session) -> Dict:
        ...
        # Update governance tokens
        governance_tokens = (
            db.query(GovernanceToken).filter(GovernanceToken.user_id == user.id).first()
        )

        if governance_tokens:
            governance_tokens.token_amount += tokens
        else:
            governance_tokens = GovernanceToken(user_id=user.id, token_amount=tokens)
            db.add(governance_tokens)
    ...
    def _check_volume_milestones(self, user: User, db: Session) -> Optional[Dict]:
        ...
        for milestone in milestones:
            if user.total_volume >= milestone and not self._milestone_awarded(
                user.id, "volume", milestone, db
            ):
    ...
    def _check_profit_milestones(self, user: User, db: Session) -> Optional[Dict]:
        ...
        for milestone in milestones:
            if user.total_profit >= milestone and not self._milestone_awarded(
                user.id, "profit", milestone, db
            ):
                tokens = milestone // 10
                xp = milestone // 5
    ...
    def _milestone_awarded(
        self, user_id: int, milestone_type: str, value: int, db: Session
    ) -> bool:
        ...
        try:
            ...
            if market_count == 1:
                ...
                if governance_tokens:
                    ...
                    governance_tokens = GovernanceToken(
                        user_id=user_id, token_amount=tokens
                    )
                    db.add(governance_tokens)
    ...
    def get_user_achievements(self, user_id: int) -> Dict:
        ...
        try:
            ...
            for achievement_id, achievement in Achievement.ACHIEVEMENTS.items():
                ...
                progress_data = self._get_achievement_progress(
                    user_id, achievement_id, db
                )
    ...
    def _check_achievement(
        self, user_id: int, achievement_id: str, db: Session
    ) -> bool:
