...
class MarketMicrostructureService:
    ...
    async def get_market_impact(self, symbol: str, limit: int = 100) -> List[MarketImpact]:
    ...
    async def create_market_making_strategy(self, symbol: str, user_id: int,
                                          strategy_type: str, parameters: Dict[str, Any]) -> MarketMakingStrategy:
    ...
    async def get_market_making_quotes(self, strategy_id: str) -> List[MarketMakingQuote]:
    ...
    async def update_market_making_parameters(self, strategy_id: str, parameters: Dict[str, Any]) -> bool:
    ...
    async def calculate_optimal_spread(self, symbol: str, trade_size: float) -> Dict[str, float]:
        ...
        try:
            ...
            optimal_spread = base_spread * size_adjustment * volatility_adjustment * liquidity_adjustment
    ...
    async def _calculate_symbol_liquidity_metrics(self, symbol: str, depth: MarketDepth):
        ...
        try:
            ...
            # Liquidity score
            liquidity_score = depth.depth_score * (1 - bid_ask_spread / 0.1)  # Normalize by spread
    ...
    async def _detect_symbol_market_regime(self, symbol: str):
        ...
        try:
            ...
            if len(price_data) >= 10:
                ...
                mean_reversion_strength = np.mean([abs(p - mean_price) / mean_price for p in recent_prices])
            else:
    ...
    async def _update_strategy_quotes(self, strategy: MarketMakingStrategy):
        ...
        try:
            ...
            # Remove old quotes
            self.active_quotes[strategy.strategy_id] = [
                q for q in self.active_quotes[strategy.strategy_id] 
...
# Factory function
async def get_market_microstructure_service(redis_client: redis.Redis, db_session: Session) -> MarketMicrostructureService:
