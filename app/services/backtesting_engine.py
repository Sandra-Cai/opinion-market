...
class BacktestingEngine:
    ...
    async def _execute_backtest(self, backtest_id: str, config: BacktestConfig):
        ...
        try:
            ...
            await self._run_strategy_backtest(
                backtest_id, config, price_data, benchmark_data
            )
        ...
        try:
            ...
            for i, current_date in enumerate(date_range):
                ...
                await self._execute_trades(
                    backtest_id, config, signals, current_prices, current_date
                )

                # Update portfolio
                await self._update_portfolio(
                    backtest_id, config, current_prices, current_date
                )
    ...
    async def _generate_signals(
        self, config: BacktestConfig, price_data: pd.DataFrame, current_date: datetime
    ) -> Dict[str, float]:
        ...
        try:
            ...
            if config.strategy_type == StrategyType.MOMENTUM:
                signals = await self._generate_momentum_signals(
                    config, price_data, current_date
                )
                ...
                signals = await self._generate_market_neutral_signals(
                    config, price_data, current_date
                )
            else:
                signals = await self._generate_default_signals(
                    config, price_data, current_date
                )
    ...
    async def _generate_market_neutral_signals(
        self, config: BacktestConfig, price_data: pd.DataFrame, current_date: datetime
    ) -> Dict[str, float]:
        ...
        try:
            ...
            for symbol in config.universe:
                ...
                universe_returns = (
                    price_data[config.universe].pct_change().iloc[-20:].mean().mean()
                )
                relative_strengths[symbol] = symbol_returns - universe_returns

            # Rank and generate signals
            sorted_symbols = sorted(
                relative_strengths.items(), key=lambda x: x[1], reverse=True
            )
    ...
    async def _generate_default_signals(
        self, config: BacktestConfig, price_data: pd.DataFrame, current_date: datetime
    ) -> Dict[str, float]:
        ...
        try:
            ...
            for symbol, signal in signals.items():
                ...
                if abs(trade_shares) > 0.01:  # Minimum trade size
                    ...
                    # Update portfolio
                    trade_cost = (
                        trade_shares * current_price + trade.commission + trade.slippage
                    )
    ...
    async def _update_portfolio(
        ...
        try:
            ...
            if len(self.portfolios[backtest_id]) > 1:
                ...
                daily_return = (
                    total_value - prev_portfolio.total_value
                ) / prev_portfolio.total_value
                cumulative_return = (
                    total_value - config.initial_capital
    ...
    async def _check_risk_limits(
        self, backtest_id: str, config: BacktestConfig, current_date: datetime
    ):
        """Check risk limits"""
        try:
    ...
    async def _calculate_final_metrics(
        self, backtest_id: str, config: BacktestConfig, benchmark_data: pd.DataFrame
    ):
        ...
        try:
            ...
            if returns:
                ...
                result.expected_shortfall = (
                    np.mean(var_95_returns) if var_95_returns else 0
                )
            ...
            if not benchmark_data.empty:
                ...
                benchmark_total_return = (
                    benchmark_data.iloc[-1] / benchmark_data.iloc[0]
                ) - 1
                ...
                # Beta and Alpha
                if len(portfolio_returns_series) == len(benchmark_returns):
                    covariance = np.cov(portfolio_returns_series, benchmark_returns)[
                        0, 1
                    ]
                    ...
                    result.alpha = result.annualized_return - (
                        result.beta * benchmark_total_return
                    )
            ...
            try:
                ...
                for backtest_id, result in self.backtest_results.items():
                    if (
                        ...
                        and result.end_time
                        and result.end_time < cutoff_date
                    ):
