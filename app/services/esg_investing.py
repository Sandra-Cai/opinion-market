...
class ESGInvestingService:
    ...
    def __init__(self, redis_client: redis.Redis, db_session: Session):
        ...
        self.sustainable_investments: Dict[str, List[SustainableInvestment]] = defaultdict(list)
    ...
    async def create_esg_score(self, company_id: str, overall_score: float,
                             environmental_score: float, social_score: float,
                             governance_score: float, framework: SustainabilityFramework,
                             data_quality: float = 0.8, coverage_percentage: float = 0.9) -> ESGScore:
        """Create ESG score"""
        try:
    ...
    async def create_esg_factor(self, category: ESGCategory, factor_name: str,
                              description: str, weight: float, measurement_unit: str,
                              data_source: str, is_material: bool = True,
                              impact_type: ImpactType = ImpactType.NEUTRAL) -> ESGFactor:
        """Create ESG factor"""
        try:
    ...
    async def create_esg_impact(self, company_id: str, factor_id: str,
                              impact_value: float, impact_unit: str,
                              impact_type: ImpactType, baseline_value: float = 0.0,
                              target_value: float = 0.0) -> ESGImpact:
        """Create ESG impact"""
        try:
            ...
            if target_value > 0 and baseline_value >= 0:
                if impact_type == ImpactType.POSITIVE:
                    progress_percentage = (impact_value - baseline_value) / (target_value - baseline_value) * 100
                else:
                    progress_percentage = (baseline_value - impact_value) / (baseline_value - target_value) * 100
    ...
    async def create_sustainable_investment(self, user_id: int, company_id: str,
                                          investment_amount: float, esg_score: float,
                                          sustainability_rating: str,
                                          impact_metrics: Dict[str, float],
                                          alignment_score: float, risk_score: float,
                                          expected_return: float) -> SustainableInvestment:
    ...
    async def create_esg_portfolio(self, user_id: int, portfolio_name: str) -> ESGPortfolio:
        ...
        try:
            ...
            esg_score = np.mean([inv.esg_score for inv in investments]) if investments else 0.0
            carbon_footprint = sum(inv.impact_metrics.get('carbon_footprint', 0) for inv in investments)
            ...
            risk_score = np.mean([inv.risk_score for inv in investments]) if investments else 0.0
            expected_return = np.mean([inv.expected_return for inv in investments]) if investments else 0.0
            
            # Calculate benchmark comparison
            benchmark_comparison = await self._calculate_benchmark_comparison(esg_score, expected_return)
    ...
    async def create_impact_report(self, user_id: int, report_period: str) -> ImpactReport:
        ...
        try:
            ...
            # Calculate impact metrics
            environmental_impact = await self._calculate_environmental_impact(investments)
            ...
            carbon_avoided = sum(inv.impact_metrics.get('carbon_avoided', 0) for inv in investments)
            jobs_created = sum(int(inv.impact_metrics.get('jobs_created', 0)) for inv in investments)
            community_benefits = sum(inv.impact_metrics.get('community_benefits', 0) for inv in investments)
            
            # Calculate sustainability goals achieved
            sustainability_goals_achieved = await self._calculate_sustainability_goals_achieved(investments)
    ...
    async def create_esg_alert(self, company_id: str, alert_type: str, severity: str,
                             description: str, impact_score: float, recommendation: str) -> ESGAlert:
        """Create ESG alert"""
        try:
    ...
    async def get_esg_analytics(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate impact metrics
            total_carbon_avoided = sum(inv.impact_metrics.get('carbon_avoided', 0) for inv in investments)
            total_jobs_created = sum(int(inv.impact_metrics.get('jobs_created', 0)) for inv in investments)
            total_community_benefits = sum(inv.impact_metrics.get('community_benefits', 0) for inv in investments)
            ...
            benchmark_comparison = await self._calculate_benchmark_comparison(avg_esg_score, avg_expected_return)
            
            # Calculate sustainability goals progress
            sustainability_progress = await self._calculate_sustainability_progress(investments)
    ...
    async def _update_investment_impact_metrics(self, investment: SustainableInvestment):
        """Update investment impact metrics"""
        try:
            # Simulate impact metric updates
            investment.impact_metrics.update({
                'carbon_avoided': investment.investment_amount * 0.1,  # 0.1 tons per $1000
                'jobs_created': int(investment.investment_amount / 100000),  # 1 job per $100k
                'community_benefits': investment.investment_amount * 0.05,  # 5% community benefit
                'renewable_energy': investment.investment_amount * 0.2,  # 20% renewable energy
                'water_saved': investment.investment_amount * 0.5,  # 0.5 gallons per $1000
                'waste_reduced': investment.investment_amount * 0.3  # 0.3 tons per $1000
            })
    ...
    async def _update_portfolio_metrics(self, portfolio: ESGPortfolio):
        ...
        try:
            ...
            portfolio.esg_score = np.mean([inv.esg_score for inv in investments]) if investments else 0.0
            portfolio.carbon_footprint = sum(inv.impact_metrics.get('carbon_footprint', 0) for inv in investments)
            portfolio.impact_metrics = await self._calculate_portfolio_impact_metrics(investments)
            portfolio.sustainability_rating = self._calculate_sustainability_rating(portfolio.esg_score)
            portfolio.risk_score = np.mean([inv.risk_score for inv in investments]) if investments else 0.0
            portfolio.expected_return = np.mean([inv.expected_return for inv in investments]) if investments else 0.0
            portfolio.benchmark_comparison = await self._calculate_benchmark_comparison(portfolio.esg_score, portfolio.expected_return)
    ...
    async def _calculate_portfolio_impact_metrics(self, investments: List[SustainableInvestment]) -> Dict[str, float]:
    ...
    async def _calculate_benchmark_comparison(self, esg_score: float, expected_return: float) -> Dict[str, float]:
        """Calculate benchmark comparison"""
        try:
            # Get benchmark data
            ...
            return {
                ...
                'return_percentile': (expected_return / 0.15) * 100  # 15% max return assumption
            }
    ...
    async def _calculate_environmental_impact(self, investments: List[SustainableInvestment]) -> Dict[str, float]:
        ...
        try:
            ...
            for investment in investments:
                impact['carbon_avoided'] += investment.impact_metrics.get('carbon_avoided', 0)
                impact['renewable_energy'] += investment.impact_metrics.get('renewable_energy', 0)
                impact['water_saved'] += investment.impact_metrics.get('water_saved', 0)
                impact['waste_reduced'] += investment.impact_metrics.get('waste_reduced', 0)
                impact['biodiversity_protected'] += investment.impact_metrics.get('biodiversity_protected', 0)
    ...
    async def _calculate_social_impact(self, investments: List[SustainableInvestment]) -> Dict[str, float]:
        ...
        try:
            ...
            for investment in investments:
                impact['jobs_created'] += investment.impact_metrics.get('jobs_created', 0)
                impact['community_benefits'] += investment.impact_metrics.get('community_benefits', 0)
                impact['education_supported'] += investment.impact_metrics.get('education_supported', 0)
                impact['healthcare_improved'] += investment.impact_metrics.get('healthcare_improved', 0)
                impact['diversity_promoted'] += investment.impact_metrics.get('diversity_promoted', 0)
    ...
    async def _calculate_governance_impact(self, investments: List[SustainableInvestment]) -> Dict[str, float]:
        ...
        try:
            ...
            for investment in investments:
                impact['transparency_improved'] += investment.impact_metrics.get('transparency_improved', 0)
                impact['ethics_promoted'] += investment.impact_metrics.get('ethics_promoted', 0)
                impact['stakeholder_engagement'] += investment.impact_metrics.get('stakeholder_engagement', 0)
                impact['risk_management'] += investment.impact_metrics.get('risk_management', 0)
                impact['compliance_enhanced'] += investment.impact_metrics.get('compliance_enhanced', 0)
    ...
    async def _calculate_sustainability_goals_achieved(self, investments: List[SustainableInvestment]) -> List[str]:
        """Calculate sustainability goals achieved"""
        try:
            goals = []
            ...
            total_renewable_energy = sum(inv.impact_metrics.get('renewable_energy', 0) for inv in investments)
    ...
    async def _calculate_sustainability_progress(self, investments: List[SustainableInvestment]) -> Dict[str, float]:
        ...
        try:
            ...
            progress['carbon_reduction'] = sum(inv.impact_metrics.get('carbon_avoided', 0) for inv in investments) / total_investment * 1000
            progress['renewable_energy_adoption'] = sum(inv.impact_metrics.get('renewable_energy', 0) for inv in investments) / total_investment * 100
            progress['social_impact'] = sum(inv.impact_metrics.get('jobs_created', 0) for inv in investments) / total_investment * 100000
...
# Factory function
async def get_esg_investing_service(redis_client: redis.Redis, db_session: Session) -> ESGInvestingService:
