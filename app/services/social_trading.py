...
class SocialTradingService:
    ...
    def __init__(self, redis_client: redis_sync.Redis, db_session: Session):
        ...
        self.signal_performance: Dict[str, deque] = defaultdict(
    ...
    async def update_trader_performance(
        self, user_id: int, trade_result: Dict[str, Any]
    ):
        ...
        try:
            ...
            if trade_result.get("profit_loss", 0.0) > 0:
                ...
                profile.win_rate = (
                    profile.win_rate * (profile.total_trades - 1) / profile.total_trades
                )
    ...
    async def unfollow_trader(self, follower_id: int, leader_id: int) -> bool:
        ...
        try:
            if (
                leader_id in self.follower_relationships
                and follower_id in self.follower_relationships[leader_id]
            ):
                self.follower_relationships[leader_id].remove(follower_id)

                # Update follower counts
                if leader_id in self.trader_profiles:
                    self.trader_profiles[leader_id].followers_count = max(
                        0, self.trader_profiles[leader_id].followers_count - 1
                    )
                    await self._cache_trader_profile(self.trader_profiles[leader_id])

                if follower_id in self.trader_profiles:
                    self.trader_profiles[follower_id].following_count = max(
                        0, self.trader_profiles[follower_id].following_count - 1
                    )
                    await self._cache_trader_profile(self.trader_profiles[follower_id])
    ...
    async def get_leaderboard(
        ...
        try:
            ...
            await self.redis.setex(
                cache_key, 3600, json.dumps(leaderboard_data)
            )  # Cache for 1 hour
        ...
        try:
            ...
            for post in self.community_posts.values():
                if post.user_id in followed_traders:
                    feed_items.append(
                        {"type": "post", "data": post, "timestamp": post.created_at}
                    )
            ...
            analytics = {
                ...
                "social": {
                    "signals_created": len(
                        [
                            s
                            for s in self.social_signals.values()
                            if s.trader_id == user_id
                        ]
                    ),
                    "signals_followed": len(
                        [
                            s
                            for s in self.social_signals.values()
                            if s.followers_count > 0
                        ]
                    ),
                    "posts_created": len(
                        [
                            p
                            for p in self.community_posts.values()
                    ...
                    "copy_trades": len(
                        [
                            c
                            for c in self.copy_trades.values()
                            if c.follower_id == user_id
                        ]
                    ),
                },
                "markets": {
                    ...
                    "best_performing_market": await self._get_best_performing_market(
                        user_id
                    ),
                },
            }
    ...
    async def _track_performance(self):
        ...
        while True:
            try:
                ...
                for user_id, profile in self.trader_profiles.items():
                    profile.performance_rating = self._calculate_performance_rating(
                        profile
                    )
                    await self._cache_trader_profile(profile)
