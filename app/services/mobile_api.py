...
class MobileAPIService:
    ...
    def get_mobile_dashboard(self, user_id: int) -> Dict:
        ...
        try:
            ...
            recent_markets = db.query(Market).filter(
                Market.status == 'open'
            ).order_by(desc(Market.created_at)).limit(10).all()
            ...
            trending_markets = db.query(Market).filter(
                Market.status == 'open'
            ).order_by(desc(Market.trending_score)).limit(5).all()
    ...
    def get_mobile_market_details(self, market_id: int, user_id: Optional[int] = None) -> Dict:
                ...
                "recent_trades": [
                    {
                        ...
                        "created_at": trade.created_at,
                        "user_username": trade.user.username if trade.user else "Anonymous"
                    }
                    for trade in recent_trades
                ],
    ...
    def _calculate_price_history(self, market_id: int, db: Session) -> List[Dict]:
        ...
        price_history = []
        if trades:
            current_hour = trades[0].created_at.replace(minute=0, second=0, microsecond=0)
            end_time = datetime.utcnow()
            
            while current_hour <= end_time:
                hour_trades = [t for t in trades if 
                             t.created_at >= current_hour and 
                             t.created_at < current_hour + timedelta(hours=1)]
                
                if hour_trades:
                    outcome_a_trades = [t for t in hour_trades if t.outcome == "outcome_a"]
                    outcome_b_trades = [t for t in hour_trades if t.outcome == "outcome_b"]
                    
                    avg_price_a = sum(t.price_per_share for t in outcome_a_trades) / len(outcome_a_trades) if outcome_a_trades else 0
                    avg_price_b = sum(t.price_per_share for t in outcome_b_trades) / len(outcome_b_trades) if outcome_b_trades else 0
    ...
    def get_mobile_leaderboard(self, category: str = "traders", period: str = "7d", limit: int = 20) -> Dict:
        ...
        try:
            if category == "traders":
                ...
                users = db.query(User).filter(
                    User.is_active == True
                ).order_by(desc(User.total_profit)).limit(limit).all()
                ...
                users = db.query(User).filter(
                    User.is_active == True
                ).order_by(desc(User.total_volume)).limit(limit).all()
                ...
                ).order_by(desc(User.win_rate)).limit(limit).all()
