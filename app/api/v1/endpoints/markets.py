...
def get_markets(
    ...
    return MarketListResponse(
        markets=markets, total=total, page=skip // limit + 1, per_page=limit
    )
...
def get_trending_markets(
    ...
    trending_markets = (
        db.query(Market)
        .filter(Market.status == MarketStatus.OPEN, Market.created_at >= yesterday)
...
def get_market_stats(db: Session = Depends(get_db)):
    ...
    most_active_category = (
        db.query(Market.category, func.count(Market.id).label("count"))
        .group_by(Market.category)
        .order_by(desc("count"))
        .first()
    )
    ...
    return MarketStats(
        ...
        most_active_category=(
            most_active_category[0] if most_active_category else "other"
        ),
