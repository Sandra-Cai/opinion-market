...
class SocialFeaturesService:
    ...
    async def create_user_profile(self, user_id: int, profile_data: Dict[str, Any]) -> Optional[UserProfile]:
        ...
        try:
            ...
            reputation_score = await self._calculate_reputation_score(user_id, trading_stats)
    ...
    async def create_social_post(self, user_id: int, post_data: Dict[str, Any]) -> Optional[SocialPost]:
    ...
    async def create_community(self, owner_id: int, community_data: Dict[str, Any]) -> Optional[Community]:
    ...
    async def get_user_feed(self, user_id: int, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for post in feed_posts:
                post['user_has_liked'] = await self._has_liked_post(user_id, post['post_id'])
                post['user_has_shared'] = await self._has_shared_post(user_id, post['post_id'])
        ...
        try:
            ...
            for hashtag, score in trending_hashtags:
                trending_topics.append({
                    ...
                    'score': score,
                    'post_count': await self._get_hashtag_post_count(hashtag.decode())
                })
    ...
    async def _calculate_trading_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            profit_loss = sum(trade.total_value * 0.1 for trade in trades)  # Placeholder
    ...
    async def _calculate_reputation_score(self, user_id: int, trading_stats: Dict[str, Any]) -> float:
        ...
        try:
            ...
            community_score = await self._calculate_community_contribution_score(user_id)
    ...
    async def _is_following(self, follower_id: int, followed_id: int) -> bool:
        ...
        try:
            ...
            return await self.redis_client.sismember(f"followers:{followed_id}", follower_id)
    ...
    async def _add_to_follower_feed(self, follower_id: int, followed_id: int):
        ...
        try:
            ...
            recent_posts = await self.redis_client.lrange(f"user_feed:{followed_id}", 0, 9)
    ...
    async def _remove_from_follower_feed(self, follower_id: int, followed_id: int):
        ...
        try:
            ...
            user_posts = await self.redis_client.lrange(f"user_feed:{followed_id}", 0, -1)
    ...
    async def _send_follow_notification(self, followed_id: int, follower_id: int):
        ...
        logger.info(f"Follow notification sent to user {followed_id} from user {follower_id}")
    ...
    async def _send_like_notification(self, post_user_id: int, liker_id: int, post_id: str):
        """Send like notification"""
        # In a real implementation, you would send a notification
        logger.info(f"Like notification sent to user {post_user_id} from user {liker_id}")
    ...
    async def _get_feed_posts(self, user_id: int, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            post_ids = await self.redis_client.lrange(f"user_feed:{user_id}", 0, limit - 1)
    ...
    async def _calculate_engagement_rate(self, user_id: int) -> float:
        ...
        try:
            # Get user's posts
            user_posts = [post for post in self.social_posts.values() if post.user_id == user_id]
            ...
            engagement_rate = (total_engagement / len(user_posts)) / followers_count * 100
    ...
    async def _get_content_performance(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            best_post = max(user_posts, key=lambda p: p.likes_count + p.comments_count + p.shares_count)
    ...
    async def _calculate_community_contribution_score(self, user_id: int) -> float:
        ...
        try:
            ...
            user_posts = len([post for post in self.social_posts.values() if post.user_id == user_id])
    ...
    async def _add_community_member(self, community_id: str, user_id: int, role: str):
        ...
        try:
            ...
            await self.redis_client.hset(f"community_roles:{community_id}", user_id, role)
