...
def create_user_profile(
    profile_data: Dict = Body(...), current_user: User = Depends(get_current_user)
):
    ...
    # Create profile
    profile = asyncio.run(
        social_features.create_user_profile(current_user.id, profile_data)
    )
...
def get_user_profile(
    user_id: int, current_user: Optional[User] = Depends(get_current_user)
):
    """Get user profile"""
    social_features = get_social_features()
...
def create_social_post(
    post_data: Dict = Body(...), current_user: User = Depends(get_current_user)
):
...
def get_social_posts(
    ...
    return {
        "posts": posts,
        "total": len(posts),
        "filters": {"user_id": user_id, "post_type": post_type},
    }
...
def like_post(post_id: str, current_user: User = Depends(get_current_user)):
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to like post"
        )
...
def get_user_feed(
    ...
    return {"posts": feed_posts, "total": len(feed_posts), "user_id": current_user.id}


@router.post("/follow/{user_id}")
def follow_user(user_id: int, current_user: User = Depends(get_current_user)):
    """Follow a user"""
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to follow user"
        )
...
def unfollow_user(user_id: int, current_user: User = Depends(get_current_user)):
    """Unfollow a user"""
    social_features = get_social_features()
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to unfollow user"
        )
...
def create_community(
    community_data: Dict = Body(...), current_user: User = Depends(get_current_user)
):
    ...
    community = asyncio.run(
        social_features.create_community(current_user.id, community_data)
    )

    if not community:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to create community"
        )
...
def get_communities(
    ...
    return {
        ...
        "filters": {"owner_id": owner_id, "is_private": is_private},
    }
...
def get_trending_topics(
    ...
    return {"trending_topics": trending_topics, "total": len(trending_topics)}


@router.get("/analytics")
def get_social_analytics(current_user: User = Depends(get_current_user)):
...
def get_user_followers(
    ...
    followers = asyncio.run(
        social_features.redis_client.smembers(f"followers:{user_id}")
    )
...
def get_user_following(
    ...
    following = asyncio.run(
        social_features.redis_client.smembers(f"following:{user_id}")
    )
...
def get_social_post(
    post_id: str, current_user: Optional[User] = Depends(get_current_user)
):
    ...
    if current_user:
        user_has_liked = asyncio.run(
            social_features._has_liked_post(current_user.id, post_id)
        )
...
def get_community(
    community_id: str, current_user: Optional[User] = Depends(get_current_user)
):
    ...
    if current_user and social_features.redis_client:
        is_member = asyncio.run(
            social_features.redis_client.sismember(
                f"community_members:{community_id}", current_user.id
            )
        )
...
def search_social_content(
    ...
    search_type: str = Query(
        "all", description="Search type: all, users, posts, communities"
    ),
    ...
    results = {"users": [], "posts": [], "communities": [], "total": 0}
    ...
    if search_type in ["all", "users"]:
        for profile in social_features.user_profiles.values():
            if (
                query_lower in profile.display_name.lower()
                or query_lower in profile.bio.lower()
                or query_lower in profile.username.lower()
            ):
    ...
    if search_type in ["all", "communities"]:
        for community in social_features.communities.values():
            if (
                query_lower in community.name.lower()
                or query_lower in community.description.lower()
            ):
    ...
    # Calculate total
    results["total"] = (
        len(results["users"]) + len(results["posts"]) + len(results["communities"])
    )
