...
class PerformanceMonitoringService:
    ...
    async def _monitor_application_metrics(self):
        ...
        while True:
            try:
                ...
                await self._record_metric('active_connections', active_connections, 'connections')
                ...
                await self._record_metric('process_memory', process_memory.rss / (1024**2), 'MB')
                
                # Garbage collection stats
                gc_stats = gc.get_stats()
                if gc_stats:
                    total_collections = sum(stat['collections'] for stat in gc_stats)
                    await self._record_metric('gc_collections', total_collections, 'count')
    ...
    async def _monitor_database_metrics(self):
        ...
        while True:
            try:
                ...
                await self._record_metric('db_connections_active', pool_stats.get('active', 0), 'connections')
                await self._record_metric('db_connections_idle', pool_stats.get('idle', 0), 'connections')
    ...
    async def _monitor_cache_metrics(self):
        ...
        while True:
            try:
                ...
                await self._record_metric('cache_hit_rate', cache_stats.get('hit_rate', 0), '%')
                await self._record_metric('cache_memory_usage', cache_stats.get('memory_usage', 0), 'MB')
                await self._record_metric('cache_keys', cache_stats.get('keys', 0), 'count')
    ...
    async def _generate_recommendations(self):
        ...
        while True:
            try:
                ...
                new_recommendations = await self._create_recommendations(performance_analysis)
    ...
    async def _check_thresholds(self, metric: PerformanceMetric):
        ...
        try:
            ...
            if metric.value > threshold:
                alert_id = f"{metric.metric_name}_{metric.timestamp.strftime('%Y%m%d_%H%M%S')}"
    ...
    async def _create_bottleneck(self, metric_name: str, current_value: float, 
                               baseline_value: float, deviation: float) -> Optional[PerformanceBottleneck]:
        ...
        try:
            ...
            recommendations = await self._get_bottleneck_recommendations(metric_name, component, current_value)
    ...
    async def _get_bottleneck_recommendations(self, metric_name: str, component: str, 
    ...
    async def _analyze_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_bottlenecks = [b for b in self.bottlenecks.values() 
                                if (datetime.utcnow() - b.detected_at).total_seconds() < 3600]
            analysis['bottlenecks'] = {b.bottleneck_id: {
    ...
    async def _create_recommendations(self, performance_analysis: Dict[str, Any]) -> List[OptimizationRecommendation]:
        ...
        try:
            ...
            if current_metrics.get('cpu_usage', 0) > 80:
                recommendations.append(OptimizationRecommendation(
                    ...
                    priority='high' if current_metrics['cpu_usage'] > 90 else 'medium',
            ...
            if current_metrics.get('memory_usage', 0) > 85:
                recommendations.append(OptimizationRecommendation(
                    ...
                    category='infrastructure',
                    priority='high' if current_metrics['memory_usage'] > 95 else 'medium',
    ...
    async def _calculate_trend(self, metric_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if len(recent_values) >= 2:
                ...
                trend_magnitude = abs(recent_values[-1] - recent_values[0]) / recent_values[0] if recent_values[0] > 0 else 0
    ...
    async def _cleanup_old_recommendations(self):
        ...
        try:
            ...
            old_recommendations = [
                rid for rid, recommendation in self.recommendations.items()
...
# Factory function
async def get_performance_monitoring_service(redis_client: redis.Redis, db_session: Session) -> PerformanceMonitoringService:
