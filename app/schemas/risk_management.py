...
class RiskProfileRequest(BaseModel):
    ...
    max_portfolio_risk: float = Field(default=0.02, ge=0.001, le=0.1, description="Maximum portfolio risk percentage")
    max_position_size: float = Field(default=0.05, ge=0.01, le=0.5, description="Maximum position size percentage")
    max_drawdown: float = Field(default=0.15, ge=0.05, le=0.5, description="Maximum acceptable drawdown")
    stop_loss_percentage: float = Field(default=0.10, ge=0.01, le=0.5, description="Default stop loss percentage")
    take_profit_percentage: float = Field(default=0.20, ge=0.01, le=1.0, description="Default take profit percentage")
    correlation_threshold: float = Field(default=0.7, ge=0.1, le=1.0, description="Maximum correlation between positions")
    volatility_preference: str = Field(default="medium", description="Volatility preference (low, medium, high)")
...
class RiskProfileResponse(BaseModel):
    ...
    take_profit_percentage: float = Field(..., description="Default take profit percentage")
    correlation_threshold: float = Field(..., description="Maximum correlation between positions")
...
class PositionLimitCheckResponse(BaseModel):
    ...
    max_allowed: Optional[float] = Field(None, description="Maximum allowed position size")
    max_allowed_risk: Optional[float] = Field(None, description="Maximum allowed portfolio risk")
...
class RiskDashboardResponse(BaseModel):
    ...
    position_risks: List[Dict[str, Any]] = Field(..., description="Position risk metrics")
...
class DrawdownAnalyticsResponse(BaseModel):
    ...
    max_drawdown: float = Field(..., description="Maximum drawdown")
    current_drawdown: float = Field(..., description="Current drawdown")
    max_drawdown_percentage: float = Field(..., description="Maximum drawdown percentage")
    current_drawdown_percentage: float = Field(..., description="Current drawdown percentage")
...
class CorrelationAnalyticsResponse(BaseModel):
    ...
    diversification_score: float = Field(..., description="Diversification score")
    diversification_percentage: float = Field(..., description="Diversification percentage")
...
class RiskSimulationRequest(BaseModel):
    ...
    position_size: float = Field(..., gt=0, description="Position size")
    entry_price: float = Field(..., gt=0, description="Entry price")
    scenarios: int = Field(default=1000, ge=100, le=10000, description="Number of simulation scenarios")
...
class RiskManagementStatsResponse(BaseModel):
    ...
    total_portfolio_risks: int = Field(..., description="Total number of portfolio risks")
    ...
    alert_severity_distribution: Dict[str, int] = Field(..., description="Alert severity distribution")
    average_risk_scores: Dict[str, float] = Field(..., description="Average risk scores")
