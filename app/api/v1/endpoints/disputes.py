...
from app.schemas.dispute import DisputeCreate, DisputeUpdate, DisputeResponse, DisputeVoteCreate, DisputeListResponse
...
def create_dispute(
    ...
    existing_dispute = db.query(MarketDispute).filter(
...
def get_disputes(
    ...
    total = query.count()
    disputes = query.order_by(desc(MarketDispute.created_at)).offset(skip).limit(limit).all()
...
def get_market_disputes(market_id: int, db: Session = Depends(get_db)):
    ...
    disputes = db.query(MarketDispute).filter(
        MarketDispute.market_id == market_id
    ).order_by(desc(MarketDispute.created_at)).all()
