...
from app.schemas.position import PositionResponse, PositionListResponse, PortfolioSummary
...
def get_positions(
    ...
    total = query.count()
    positions = query.order_by(desc(Position.last_updated)).offset(skip).limit(limit).all()
...
def get_portfolio_summary(
    ...
    portfolio_return_percentage = (total_pnl / total_invested * 100) if total_invested > 0 else 0
...
def close_position(
    ...
    close_price = market.current_price_a if position.outcome == "outcome_a" else market.current_price_b
