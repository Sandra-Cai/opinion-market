...
from app.services.quantitative_trading import (
    ...
    TradingSignal, TradingStrategy, BacktestResult, PortfolioOptimization
)
from app.schemas.quantitative_trading import (
    TradingStrategyRequest, TradingStrategyResponse,
    SignalResponse, BacktestRequest, BacktestResponse,
...
async def get_trading_strategies(
    ...
    try:
        ...
        for strategy in quant_service.trading_strategies.values():
            ...
            strategies.append(TradingStrategyResponse(
                strategy_id=strategy.strategy_id,
                strategy_name=strategy.strategy_name,
                strategy_type=strategy.strategy_type,
                parameters=strategy.parameters,
...
@router.post("/strategies/{strategy_id}/signals/generate", response_model=SignalResponse)
...
async def calculate_technical_indicators(
    ...
    try:
        ...
        calculated_indicators = await quant_service.calculate_technical_indicators(market_data, indicators)

        # Convert to response format
        indicator_values = {}
        for indicator_name, indicator_series in calculated_indicators.items():
            if hasattr(indicator_series, 'iloc'):
                indicator_values[indicator_name] = indicator_series.iloc[-1] if len(indicator_series) > 0 else None
            else:
...
async def get_quantitative_trading_stats(
    ...
    try:
        ...
        stats = {
            ...
            "active_strategies": len([s for s in quant_service.trading_strategies.values() if s.is_active]),
            ...
            "active_signals": len([s for s in quant_service.trading_signals.values() if s.expires_at > datetime.utcnow()]),
        ...
        raise HTTPException(status_code=500, detail="Error getting quantitative trading stats")
