...
async def optimize_system(
    ...
    try:
        ...
        optimization_result = await ai_optimizer.optimize_system_parameters(current_config)
...
async def get_performance_patterns(
    ...
    try:
        ...
        # Categorize patterns
        pattern_categories = {
            "performance": [p for p in patterns if "response_time" in p.get("type", "")],
...
async def _generate_predictions(metrics: Dict[str, Any], prediction_type: str, horizon: str) -> Dict[str, Any]:
    ...
    if prediction_type == "performance":
        base_value = metrics.get("timers", {}).get("http_request_duration", {}).get("avg", 0.1)
    ...
    return {
        ...
        "trend": "increasing" if variation > 1.1 else "decreasing" if variation < 0.9 else "stable",
        "uncertainty_range": {
...
async def _calculate_ai_health_score(metrics: Dict[str, Any]) -> Dict[str, Any]:
    ...
    response_time = metrics.get("timers", {}).get("http_request_duration", {}).get("avg", 0.1)
