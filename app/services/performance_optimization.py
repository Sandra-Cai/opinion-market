...
class PerformanceOptimizer:
    ...
    async def _generate_cache_key(self, func_name: str, args: tuple, kwargs: dict, prefix: str) -> str:
    ...
    async def _record_error(self, func_name: str, error_message: str):
        ...
        try:
            ...
            await self.redis_client.ltrim(f"error_log:{func_name}", 0, 99)  # Keep last 100
    ...
    async def _record_database_query(self, query_type: str, execution_time: float):
        ...
        try:
            ...
            if current_avg:
                current_avg = float(current_avg)
                total_queries = int(await self.redis_client.get("db:total_queries") or 0)
    ...
    async def _record_database_error(self, func_name: str, error_message: str):
        ...
        try:
            ...
            await self.redis_client.lpush("db:error_log", f"{func_name}: {error_message}")
    ...
    async def _generate_optimization_recommendations(self, slow_queries: List, query_patterns: Dict) -> List[str]:
        """Generate optimization recommendations"""
        recommendations = []
        
        if slow_queries:
            recommendations.append("Consider adding database indexes for frequently slow queries")
        
        if query_patterns.get("read_queries", 0) > query_patterns.get("write_queries", 0) * 10:
            recommendations.append("Consider implementing read replicas for better read performance")
        ...
        while True:
            try:
                ...
                metrics = DatabaseMetrics(
                    ...
                    total_queries=int(await self.redis_client.get("db:total_queries") or 0),
                    slow_queries=int(await self.redis_client.get("db:slow_queries") or 0),
                    avg_query_time=float(await self.redis_client.get("db:avg_query_time") or 0),
                    connection_errors=int(await self.redis_client.get("db:connection_errors") or 0),
                    timestamp=datetime.utcnow()
                )
    ...
    async def _calculate_performance_score(self, cache_metrics: Dict, db_metrics: Dict, system_resources: Dict) -> float:
