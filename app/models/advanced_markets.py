...
class FuturesContract(Base):
    ...
    def calculate_margin_requirement(self, position_size: float, current_price: float) -> float:
...
class FuturesPosition(Base):
    ...
    def update_pnl(self, current_price: float):
        ...
        if self.net_position > 0:  # Long position
            self.unrealized_pnl = (current_price - self.average_entry_price) * self.net_position
        elif self.net_position < 0:  # Short position
            self.unrealized_pnl = (self.average_entry_price - current_price) * abs(self.net_position)
        else:
...
class OptionsPosition(Base):
    ...
    def update_pnl(self, current_premium: float):
        """Update unrealized P&L"""
        if self.net_position > 0:  # Long position
            self.unrealized_pnl = (current_premium - self.average_entry_price) * self.net_position
        elif self.net_position < 0:  # Short position
            self.unrealized_pnl = (self.average_entry_price - current_premium) * abs(self.net_position)
        else:
            self.unrealized_pnl = 0.0
...
class ConditionalMarket(Base):
    ...
    trigger_market_id = Column(Integer, ForeignKey("markets.id"))  # Market that triggers this one
