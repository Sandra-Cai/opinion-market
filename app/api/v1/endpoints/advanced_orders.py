...
def create_conditional_order(
    ...
    result = order_manager.create_conditional_order(
        current_user.id, market_id, outcome, shares, limit_price,
        condition_market_id, condition_outcome, condition_price, condition_type
    )
...
def create_bracket_order(
    ...
    result = order_manager.create_bracket_order(
        current_user.id, market_id, outcome, shares,
...
def get_order_statistics(
    ...
    advanced_orders = db.query(Order).filter(
    ...
    cancelled_orders = len([o for o in advanced_orders if o.status == OrderStatus.CANCELLED])
    ...
    for order in advanced_orders:
        if order.status == OrderStatus.FILLED and order.updated_at and order.created_at:
            execution_time = (order.updated_at - order.created_at).total_seconds() / 3600  # hours
    ...
    order_types = {}
    for order in advanced_orders:
        order_type = order.metadata.get("order_type", "unknown") if order.metadata else "unknown"
    ...
    return {
        ...
        "success_rate": (filled_orders / (filled_orders + cancelled_orders) * 100) if (filled_orders + cancelled_orders) > 0 else 0
    }
