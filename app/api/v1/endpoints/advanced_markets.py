...
from app.models.advanced_markets import (
    FuturesContract, FuturesPosition, OptionsContract, OptionsPosition,
    ConditionalMarket, SpreadMarket, MarketInstrument, OptionType
)
...
def get_futures_contract(contract_id: int, db: Session = Depends(get_db)):
    ...
    contract = db.query(FuturesContract).filter(FuturesContract.id == contract_id).first()
    if not contract:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Futures contract not found"
        )
    return contract
...
def create_futures_position(
    ...
    contract = db.query(FuturesContract).filter(FuturesContract.id == position_data.contract_id).first()
    if not contract:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Futures contract not found"
...
def get_futures_positions(
    ...
    ).order_by(desc(FuturesPosition.created_at)).all()
...
def create_options_position(
    ...
    # Check if contract exists and is active
    contract = db.query(OptionsContract).filter(OptionsContract.id == position_data.contract_id).first()
    if not contract:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if contract.is_expired:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Contract is expired"
        )
    
    # Calculate required balance
    total_cost = (position_data.long_contracts + position_data.short_contracts) * contract.premium
...
def create_conditional_market(
    ...
    if conditional_data.trigger_market_id:
        trigger_market = db.query(Market).filter(Market.id == conditional_data.trigger_market_id).first()
        if not trigger_market:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Trigger market not found"
            )
...
def activate_conditional_market(
    ...
    conditional = db.query(ConditionalMarket).filter(ConditionalMarket.id == conditional_id).first()
    if not conditional:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Conditional market not found"
        )
    ...
    if conditional.check_trigger_condition():
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Trigger condition not met"
        )
...
def get_market_instruments(market_id: int, db: Session = Depends(get_db)):
    ...
    # Get futures contracts
    futures_contracts = db.query(FuturesContract).filter(
        FuturesContract.market_id == market_id
    ).all()
    
    # Get options contracts
    options_contracts = db.query(OptionsContract).filter(
        OptionsContract.market_id == market_id
    ).all()
    
    # Get conditional markets
    conditional_markets = db.query(ConditionalMarket).filter(
        ConditionalMarket.market_id == market_id
    ).all()
    
    # Get spread markets
    spread_markets = db.query(SpreadMarket).filter(
        SpreadMarket.market_id == market_id
    ).all()
