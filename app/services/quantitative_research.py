...
class QuantitativeResearchService:
    ...
    async def create_research_project(self, user_id: int, project_name: str,
                                    research_type: ResearchType, description: str,
                                    parameters: Dict[str, Any]) -> ResearchProject:
    ...
    async def run_factor_analysis(self, project_id: str, factor_list: List[str],
                                universe: List[str], start_date: datetime,
                                end_date: datetime) -> ResearchResult:
        """Run factor analysis"""
        try:
            ...
            # Prepare data
            factor_data = await self._prepare_factor_data(factor_list, universe, start_date, end_date)
    ...
    async def create_factor(self, factor_name: str, factor_type: FactorType,
                          description: str, calculation_method: str,
                          parameters: Dict[str, Any], universe: List[str],
                          frequency: str = 'daily', lookback_period: int = 252) -> Factor:
    ...
    async def get_factor_exposures(self, factor_id: str, date: Optional[datetime] = None) -> List[FactorExposure]:
    ...
    async def get_factor_returns(self, factor_id: str, start_date: Optional[datetime] = None,
                               end_date: Optional[datetime] = None) -> List[FactorReturn]:
        """Get factor returns"""
        try:
    ...
    async def run_correlation_analysis(self, symbols: List[str], start_date: datetime,
    ...
    async def run_volatility_analysis(self, symbol: str, start_date: datetime,
                                    end_date: datetime, model_type: str = 'garch') -> Dict[str, Any]:
    ...
    async def run_regime_analysis(self, symbol: str, start_date: datetime,
                                end_date: datetime) -> Dict[str, Any]:
        """Run regime analysis"""
        try:
            # Get price data
            ...
            for regime in ['normal', 'high_volatility']:
                ...
                for r in regimes:
                    if r['regime'] == regime:
                        regime_data = returns[(returns.index >= r['start_date']) & 
                ...
                if regime_returns:
                    regime_stats[regime] = {
                        ...
                        'sharpe_ratio': float(np.mean(regime_returns) / np.std(regime_returns)) if np.std(regime_returns) > 0 else 0,
    ...
    async def _calculate_factor_exposures(self, factor: Factor):
        ...
        try:
            ...
            price_data = await self._get_price_data(factor.universe, start_date, end_date)
            ...
            if factor.calculation_method == 'momentum':
                factor_values = await self._calculate_momentum_factor(price_data, factor.parameters)
            elif factor.calculation_method == 'value':
                factor_values = await self._calculate_value_factor(price_data, factor.parameters)
            elif factor.calculation_method == 'volatility':
                factor_values = await self._calculate_volatility_factor(price_data, factor.parameters)
            else:
                factor_values = await self._calculate_default_factor(price_data, factor.parameters)
            ...
            for security_id, value in factor_values.items():
                ...
                # Calculate z-score
                z_score = (value - np.mean(all_values)) / np.std(all_values) if np.std(all_values) > 0 else 0
    ...
    async def _calculate_momentum_factor(self, price_data: pd.DataFrame, parameters: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            for col in price_data.columns:
                factor_values[col] = returns[col].iloc[-1] if not pd.isna(returns[col].iloc[-1]) else 0
    ...
    async def _calculate_value_factor(self, price_data: pd.DataFrame, parameters: Dict[str, Any]) -> Dict[str, float]:
    ...
    async def _calculate_volatility_factor(self, price_data: pd.DataFrame, parameters: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            factor_values = {}
            for col in price_data.columns:
                factor_values[col] = volatility[col].iloc[-1] if not pd.isna(volatility[col].iloc[-1]) else 0
    ...
    async def _calculate_default_factor(self, price_data: pd.DataFrame, parameters: Dict[str, Any]) -> Dict[str, float]:
    ...
    async def _calculate_factor_return(self, factor: Factor):
        ...
        try:
            ...
            if previous_returns:
                cumulative_return = previous_returns[-1].cumulative_return * (1 + factor_return)
            ...
            if len(self.factor_returns[factor.factor_id]) > 1000:
                self.factor_returns[factor.factor_id] = self.factor_returns[factor.factor_id][-1000:]
    ...
    async def _run_factor_analysis_project(self, project: ResearchProject):
        ...
        try:
            ...
            universe = project.parameters.get('universe', ['AAPL', 'GOOGL', 'MSFT', 'TSLA'])
            start_date = project.parameters.get('start_date', datetime.utcnow() - timedelta(days=365))
            end_date = project.parameters.get('end_date', datetime.utcnow())
            
            # Run factor analysis
            result = await self.run_factor_analysis(project.project_id, factor_list, universe, start_date, end_date)
    ...
    async def _run_correlation_analysis_project(self, project: ResearchProject):
        ...
        try:
            # Extract parameters
            symbols = project.parameters.get('symbols', ['AAPL', 'GOOGL', 'MSFT', 'TSLA'])
            start_date = project.parameters.get('start_date', datetime.utcnow() - timedelta(days=365))
            end_date = project.parameters.get('end_date', datetime.utcnow())
    ...
    async def _run_volatility_modeling_project(self, project: ResearchProject):
        ...
        try:
            ...
            # Run volatility analysis
            result = await self.run_volatility_analysis(symbol, start_date, end_date, model_type)
            ...
            research_result = ResearchResult(
                ...
                data=result,
                metrics={'volatility': result.get('realized_volatility', {}).get('mean', 0)},
                charts=[],
            ...
            for exposure in exposures:
                ...
                if signal_type != 'hold':
                    signal = StrategySignal(
                        ...
                        timestamp=datetime.utcnow(),
                        metadata={'factor_id': factor.factor_id, 'z_score': exposure.z_score}
                    )
    ...
    async def _update_factor_performance(self, factor: Factor):
        ...
        try:
            ...
            volatility = np.std([r.factor_return for r in returns[-30:]]) if len(returns) >= 30 else 0
            sharpe_ratio = np.mean([r.factor_return for r in returns[-30:]]) / volatility if volatility > 0 else 0
            max_drawdown = max([r.max_drawdown for r in returns[-30:]]) if returns else 0
            ...
            performance_metrics = {
                ...
                'sharpe_ratio': sharpe_ratio,
                'max_drawdown': max_drawdown,
                'information_ratio': np.mean([r.information_ratio for r in returns[-30:]]) if returns else 0,
                'last_updated': datetime.utcnow()
            }
    ...
    # Helper methods
    async def _prepare_factor_data(self, factor_list: List[str], universe: List[str],
    ...
    async def _run_factor_analysis_core(self, factor_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'metrics': {
                    ...
                    'analysis_period': (factor_data.get('end_date', datetime.utcnow()) - 
                                      factor_data.get('start_date', datetime.utcnow())).days
                },
                'charts': [],
                'insights': insights,
...
# Factory function
async def get_quantitative_research_service(redis_client: redis.Redis, db_session: Session) -> QuantitativeResearchService:
