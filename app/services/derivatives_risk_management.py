...
class DerivativesRiskManagementService:
    ...
    async def create_risk_limit(self, user_id: int, risk_type: RiskType,
                              limit_name: str, limit_value: float,
                              limit_type: str = 'absolute',
                              time_horizon: str = 'daily') -> RiskLimit:
        """Create a risk limit"""
        try:
    ...
    async def calculate_var(self, user_id: int, confidence_level: float = 0.95,
        ...
        try:
            ...
            if not positions:
                return {'var': 0.0, 'confidence_level': confidence_level, 'time_horizon': time_horizon}
            
            # Get historical returns
            returns = await self._get_historical_returns(positions)
            
            if method == 'historical':
                var = await self._calculate_historical_var(returns, confidence_level, time_horizon)
            elif method == 'parametric':
                var = await self._calculate_parametric_var(returns, confidence_level, time_horizon)
            elif method == 'monte_carlo':
                var = await self._calculate_monte_carlo_var(positions, confidence_level, time_horizon)
            else:
                var = await self._calculate_historical_var(returns, confidence_level, time_horizon)
            
            # Calculate Expected Shortfall
            expected_shortfall = await self._calculate_expected_shortfall(returns, confidence_level, time_horizon)
    ...
    async def run_stress_test(self, user_id: int, test_type: StressTestType,
                            test_name: str, scenarios: List[Dict[str, Any]]) -> StressTestResult:
        """Run stress test"""
        try:
    ...
    async def generate_risk_report(self, user_id: int, report_type: str = 'comprehensive') -> RiskReport:
        ...
        try:
            ...
            recent_stress_tests = self.stress_tests.get(user_id, [])[-5:]  # Last 5 tests
            ...
            summary = {
                ...
                'breached_limits': len([l for l in risk_limits if l.current_value > l.limit_value]),
                'risk_level': self._determine_risk_level(var_result, greeks_risk, concentration_risk)
            }
    ...
    async def _check_risk_limit(self, limit: RiskLimit):
        ...
        try:
            ...
            if limit.risk_type == RiskType.MARKET:
                ...
                concentration_risk = await self.calculate_concentration_risk(limit.user_id)
            ...
            if limit.current_value > limit.limit_value:
                ...
                logger.warning(f"Risk limit breached: {limit.limit_name} - {limit.current_value} > {limit.limit_value}")
    ...
    async def _calculate_historical_var(self, returns: List[float], confidence_level: float, time_horizon: int) -> float:
    ...
    async def _calculate_parametric_var(self, returns: List[float], confidence_level: float, time_horizon: int) -> float:
    ...
    async def _calculate_monte_carlo_var(self, positions: List[Dict[str, Any]], confidence_level: float, time_horizon: int) -> float:
    ...
    async def _calculate_expected_shortfall(self, returns: List[float], confidence_level: float, time_horizon: int) -> float:
    ...
    async def _run_stress_scenario(self, positions: List[Dict[str, Any]], scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for position in positions:
                ...
                if 'greeks' in position:
                    ...
                    # Gamma impact
                    gamma_impact = 0.5 * quantity * greeks.get('gamma', 0) * (market_shock ** 2)
                    ...
                    position_loss += delta_impact + gamma_impact + theta_impact + vega_impact
    ...
    async def _get_historical_returns(self, positions: List[Dict[str, Any]]) -> List[float]:
    ...
    def _determine_risk_level(self, var_result: Dict[str, Any], greeks_risk: Dict[str, Any],
                            concentration_risk: Dict[str, Any]) -> str:
    ...
    async def _generate_risk_recommendations(self, var_result: Dict[str, Any], greeks_risk: Dict[str, Any],
                                          concentration_risk: Dict[str, Any], correlation_risk: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            var = var_result.get('var', 0)
            if var > 20000:
                recommendations.append("Consider reducing portfolio exposure - VaR exceeds $20,000")
            elif var > 10000:
                recommendations.append("Monitor VaR closely - approaching high risk threshold")
            
            # Greeks recommendations
            greeks = greeks_risk.get('total_greeks_risk', 0)
            if greeks > 10000:
                recommendations.append("High Greeks risk detected - consider hedging strategies")
            ...
            if correlation > 0.3:
                recommendations.append("High correlation risk - consider uncorrelated assets")
...
# Factory function
async def get_derivatives_risk_management_service(redis_client: redis.Redis, db_session: Session) -> DerivativesRiskManagementService:
