...
async def get_comprehensive_market_analysis(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Error performing comprehensive analysis"
        )
...
async def get_portfolio_advanced_analysis(
    ...
    try:
        ...
        if not user_markets:
            raise HTTPException(
                status_code=404, detail="No active markets found for user"
            )
        ...
        portfolio_analysis = {
            ...
            "correlation_analysis": {
                "high_correlations": len(
                    [c for c in correlations if c.significance_level == "high"]
                ),
                "medium_correlations": len(
                    [c for c in correlations if c.significance_level == "medium"]
                ),
                "low_correlations": len(
                    [c for c in correlations if c.significance_level == "low"]
                ),
                "avg_correlation": (
                    np.mean([abs(c.correlation_coefficient) for c in correlations])
                    if correlations
                    else 0.0
                ),
            },
            "clustering_analysis": {
                ...
                "avg_cluster_size": (
                    np.mean([c.cluster_size for c in clusters]) if clusters else 0.0
                ),
                "avg_similarity": (
                    np.mean([c.similarity_score for c in clusters]) if clusters else 0.0
                ),
            },
            "anomaly_analysis": {
                "critical_anomalies": len(
            ...
            "forecast_analysis": {
                "bullish_forecasts": len(
                    [f for f in forecasts if f.trend_direction == "bullish"]
                ),
                "bearish_forecasts": len(
                    [f for f in forecasts if f.trend_direction == "bearish"]
                ),
                "neutral_forecasts": len(
                    [f for f in forecasts if f.trend_direction == "neutral"]
                ),
                "high_risk_forecasts": len(
                    [f for f in forecasts if f.risk_assessment == "high"]
                ),
            },
        ...
        raise HTTPException(
            status_code=500, detail="Error performing portfolio analysis"
        )
...
async def compare_markets(
    ...
    try:
            ...
            "clustering_results": [
                {
                    "cluster_id": cluster.cluster_id,
                    "market_ids": cluster.market_ids,
                    "cluster_type": cluster.cluster_characteristics.get(
        ...
        raise HTTPException(
            status_code=500, detail="Error performing market comparison"
        )
                        ...
                        json.dumps(
                            {
                                ...
                                "timestamp": datetime.utcnow().isoformat(),
                                "data": {
                                    "correlation_count": len(market_ids)
                                    * (len(market_ids) - 1)
                                    // 2,
...
def _calculate_overall_risk(
    self, anomalies: List[MarketAnomaly], forecasts: List[MarketForecast]
) -> str:
    ...
    try:
        ...
        if clusters:
            ...
            if total_markets > 0:
                ...
                metrics["diversification_score"] = 1 - (
                    np.std(cluster_sizes) / np.mean(cluster_sizes)
                )
        ...
        if correlations:
            high_correlations = [
                ...
                if c.significance_level == "high"
                and abs(c.correlation_coefficient) > 0.7
            ]
            metrics["correlation_risk"] = (
                len(high_correlations) / len(correlations) if correlations else 0.0
            )

        # Calculate anomaly risk
        if anomalies:
            critical_anomalies = [a for a in anomalies if a.severity == "critical"]
            metrics["anomaly_risk"] = (
                len(critical_anomalies) / len(anomalies) if anomalies else 0.0
            )

        # Calculate forecast optimism
        if forecasts:
            bullish_forecasts = [f for f in forecasts if f.trend_direction == "bullish"]
            metrics["forecast_optimism"] = (
                len(bullish_forecasts) / len(forecasts) if forecasts else 0.0
            )
...
def _generate_portfolio_recommendations(
    ...
    try:
        ...
        high_correlations = [
            c
            for c in correlations
            if c.significance_level == "high" and abs(c.correlation_coefficient) > 0.8
        ]
        if len(high_correlations) > 3:
            recommendations.append(
                "Consider reducing exposure to highly correlated markets to improve diversification"
            )
        ...
        if critical_anomalies:
            recommendations.append(
                f"Monitor {len(critical_anomalies)} critical anomalies in your portfolio"
            )
        ...
        if bearish_forecasts:
            recommendations.append(
                f"Consider hedging strategies for {len(bearish_forecasts)} bearish forecasts"
            )

        # Clustering-based recommendations
        if len(clusters) == 1 and clusters[0].cluster_size > 5:
            recommendations.append(
                "Your portfolio is concentrated in one market cluster - consider diversifying across different market types"
            )

        if not recommendations:
            recommendations.append(
                "Your portfolio appears well-balanced based on current analysis"
            )
...
def _create_comparison_matrix(
    ...
    try:
        ...
        for market_id in market_ids:
            ...
            market_forecast = next(
                (f for f in forecasts if f.market_id == market_id), None
            )

            matrix[market_id] = {
                "anomaly_count": len(market_anomalies),
                "critical_anomalies": len(
                    [a for a in market_anomalies if a.severity == "critical"]
                ),
                "forecast_trend": (
                    market_forecast.trend_direction if market_forecast else "unknown"
                ),
                "forecast_risk": (
                    market_forecast.risk_assessment if market_forecast else "unknown"
                ),
                "cluster_assignment": next(
                    (c.cluster_id for c in clusters if market_id in c.market_ids), None
                ),
            }
...
def _build_correlation_matrix(
    self, market_ids: List[int], correlations: List[MarketCorrelation]
                ...
                if market_id_1 == market_id_2:
                    ...
                    correlation = next(
                        (
                            ...
                            for c in correlations
                            if (
                                c.market_id_1 == market_id_1
                                and c.market_id_2 == market_id_2
                            )
                            or (
                                c.market_id_1 == market_id_2
                                and c.market_id_2 == market_id_1
                            )
                        ),
                        None,
                    )
                    matrix[market_id_1][market_id_2] = (
                        correlation.correlation_coefficient if correlation else 0.0
                    )
...
def _generate_comparison_insights(
    ...
    try:
        ...
        if high_correlations:
            insights.append(
                f"Found {len(high_correlations)} strong correlations between markets"
            )

        # Clustering insights
        if len(clusters) > 1:
            insights.append(
                f"Markets are grouped into {len(clusters)} distinct clusters"
            )
        elif len(clusters) == 1:
        ...
        if anomalies:
            insights.append(
                f"Detected {len(anomalies)} anomalies across {len(set(a.market_id for a in anomalies))} markets"
            )
        ...
        if forecasts:
            ...
            insights.append(
                f"Forecasts show {bullish_count} bullish and {bearish_count} bearish trends"
            )
