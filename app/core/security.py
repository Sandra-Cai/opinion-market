...
class FraudDetector:
    ...
    def analyze_trade(self, trade: Trade, user: User) -> Dict[str, any]:
        ...
        self.user_risk_scores[user.id] = min(
            1.0, self.user_risk_scores[user.id] + risk_score
        )
    ...
    def _get_recent_trades(self, user_id: int, minutes: int) -> List[Trade]:
        ...
        try:
            ...
            return (
                db.query(Trade)
                .filter(Trade.user_id == user_id, Trade.created_at >= cutoff_time)
                .all()
            )
        finally:
            db.close()
    ...
    def _detect_price_manipulation(self, trade: Trade) -> bool:
        ...
        if (
            trade.total_value > trade.market.total_liquidity * 0.1
        ):  # More than 10% of liquidity
            return True
...
class SecurityMonitor:
    ...
    def monitor_request(
        ...
        return {
            ...
            "user_risk_score": (
                self.fraud_detector.user_risk_scores.get(user.id, 0.0) if user else 0.0
            ),
        }
    ...
    def _check_suspicious_patterns(self, request: Request, user: User) -> float:
        ...
        if (
            self.rate_limiter.get_remaining_requests(request.client.host, "default")
            < 10
        ):
            score += 0.3
    ...
    def _create_alert(self, alert_type: str, data: Dict):
        ...
        alert = {
            ...
            "severity": (
                "high"
                if alert_type in ["suspicious_trade", "price_manipulation"]
                else "medium"
            ),
        }
