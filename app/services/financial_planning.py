...
class FinancialPlanningService:
    ...
    def __init__(self, redis_client: redis.Redis, db_session: Session):
        ...
        self.retirement_analyses: Dict[str, List[RetirementAnalysis]] = defaultdict(list)
    ...
    async def create_financial_plan(self, client_id: str, plan_type: PlanningType,
                                  plan_name: str, description: str,
                                  target_date: datetime, target_amount: float,
                                  current_amount: float, expected_return: float = 0.08) -> FinancialPlan:
        ...
        try:
            ...
            if months_to_target > 0:
                required_contribution = (target_amount - current_amount) / ((1 + monthly_rate) ** months_to_target - 1) * monthly_rate
            else:
                required_contribution = 0
            
            # Calculate probability of success
            probability_of_success = await self._calculate_success_probability(
                current_amount, required_contribution, target_amount, expected_return, years_to_target
            )
            
            # Generate scenarios
            scenarios = await self._generate_scenarios(current_amount, required_contribution, target_amount, expected_return, years_to_target)
            
            # Generate recommendations
            recommendations = await self._generate_recommendations(plan_type, current_amount, target_amount, required_contribution)
    ...
    async def create_cash_flow_analysis(self, client_id: str, monthly_income: float,
                                      monthly_expenses: float) -> CashFlowAnalysis:
        ...
        try:
            ...
            emergency_fund_months = (monthly_expenses * 6) / monthly_savings if monthly_savings > 0 else 0
    ...
    async def create_retirement_analysis(self, client_id: str, current_age: int,
                                       retirement_age: int, current_savings: float,
                                       annual_contribution: float,
                                       employer_match: float = 0.0,
                                       expected_return: float = 0.07,
                                       inflation_rate: float = 0.03,
                                       retirement_income_needed: float = 0.0) -> RetirementAnalysis:
        ...
        try:
            ...
            projected_savings = await self._calculate_retirement_savings(
                current_savings, total_annual_contribution, expected_return, years_to_retirement
            )
            
            # Calculate retirement income needed
            if retirement_income_needed == 0:
                retirement_income_needed = current_savings * 0.8  # 80% replacement ratio assumption
            
            # Calculate income replacement ratio
            income_replacement_ratio = retirement_income_needed / (current_savings * 12) if current_savings > 0 else 0
            
            # Calculate probability of success
            probability_of_success = await self._calculate_retirement_success_probability(
                projected_savings, retirement_income_needed, life_expectancy - retirement_age
            )
            ...
            recommendations = await self._generate_retirement_recommendations(
                projected_savings, retirement_income_needed, shortfall_amount, probability_of_success
            )
    ...
    async def create_estate_plan(self, client_id: str, total_estate_value: float,
                               liquid_assets: float, illiquid_assets: float,
                               beneficiaries: List[Dict[str, Any]],
                               will_status: str = 'none') -> EstatePlan:
        """Create estate plan"""
        try:
            ...
            tax_optimization_opportunities = await self._identify_estate_tax_opportunities(
                total_estate_value, liquid_assets, illiquid_assets
            )
            
            # Generate recommendations
            recommendations = await self._generate_estate_recommendations(
                total_estate_value, estimated_taxes, will_status, tax_optimization_opportunities
            )
    ...
    async def create_insurance_analysis(self, client_id: str, annual_income: float,
                                      dependents: int, current_life_insurance: float = 0.0,
                                      current_disability_insurance: float = 0.0,
                                      current_long_term_care: float = 0.0) -> InsuranceAnalysis:
        ...
        try:
            ...
            disability_insurance_gap = max(0, disability_insurance_needed - current_disability_insurance)
    ...
    async def create_tax_strategy(self, client_id: str, strategy_type: str,
                                description: str, potential_savings: float,
                                implementation_cost: float, risk_level: str,
                                time_horizon: str, requirements: List[str]) -> TaxStrategy:
        """Create tax strategy"""
        try:
    ...
    async def _update_plan_progress(self, plan: FinancialPlan):
        ...
        try:
            ...
            plan.current_amount = plan.current_amount * (1 + monthly_return) + monthly_contribution
    ...
    async def _check_goal_progress(self, plan: FinancialPlan):
        ...
        try:
            ...
            if years_to_target > 0:
                required_growth = (plan.target_amount - plan.current_amount) / plan.current_amount if plan.current_amount > 0 else 0
                required_annual_return = (required_growth / years_to_target) if years_to_target > 0 else 0
                
                if required_annual_return > plan.expected_return * 1.5:  # 50% above expected
                    # Goal may be at risk
                    logger.warning(f"Goal {plan.plan_id} may be at risk - required return: {required_annual_return:.2%}")
    ...
    # Helper methods
    async def _calculate_success_probability(self, current_amount: float, required_contribution: float,
                                          target_amount: float, expected_return: float, years: float) -> float:
        """Calculate probability of success"""
        try:
            ...
            for _ in range(n_simulations):
                ...
                for year in range(int(years)):
                    final_amount = final_amount * (1 + annual_returns[year]) + required_contribution * 12
    ...
    async def _generate_scenarios(self, current_amount: float, required_contribution: float,
                                target_amount: float, expected_return: float, years: float) -> Dict[str, Any]:
        """Generate scenarios"""
        try:
            scenarios = {}
            
            # Base case
            base_case_amount = current_amount * (1 + expected_return) ** years + required_contribution * 12 * years
            scenarios['base_case'] = {
            ...
            optimistic_amount = current_amount * (1 + optimistic_return) ** years + required_contribution * 12 * years
            ...
            pessimistic_amount = current_amount * (1 + pessimistic_return) ** years + required_contribution * 12 * years
    ...
    async def _generate_recommendations(self, plan_type: PlanningType, current_amount: float,
                                      target_amount: float, required_contribution: float) -> List[str]:
        """Generate recommendations"""
        try:
            recommendations = []
            ...
            if required_contribution > current_amount * 0.2:
                recommendations.append('Consider increasing income or reducing expenses')
            
            if current_amount < target_amount * 0.1:
                recommendations.append('Start saving immediately to benefit from compound growth')
    ...
    async def _calculate_cash_flow_health_score(self, savings_rate: float, emergency_fund_months: float,
                                              debt_to_income_ratio: float) -> float:
        """Calculate cash flow health score"""
        try:
            # Weighted scoring
            ...
            # Weighted average
            health_score = (savings_score * 0.4 + emergency_score * 0.3 + debt_score * 0.3)
    ...
    async def _generate_cash_flow_recommendations(self, savings_rate: float, emergency_fund_months: float,
        ...
        try:
            ...
            if not recommendations:
                recommendations.append('Cash flow is healthy - maintain current practices')
    ...
    async def _calculate_retirement_savings(self, current_savings: float, annual_contribution: float,
                                          expected_return: float, years: int) -> float:
        ...
        try:
            ...
            if annual_contribution > 0 and years > 0:
                contribution_future_value = annual_contribution * (((1 + expected_return) ** years - 1) / expected_return)
    ...
    async def _calculate_retirement_success_probability(self, projected_savings: float,
                                                      retirement_income_needed: float,
                                                      retirement_years: int) -> float:
        ...
        try:
            ...
            annual_withdrawal = retirement_income_needed / retirement_years if retirement_years > 0 else 0
            success_ratio = projected_savings / (annual_withdrawal * retirement_years) if annual_withdrawal > 0 else 0
    ...
    async def _generate_retirement_recommendations(self, projected_savings: float,
                                                 retirement_income_needed: float,
                                                 shortfall_amount: float,
                                                 probability_of_success: float) -> List[str]:
        ...
        try:
            ...
            if probability_of_success < 0.8:
                ...
                recommendations.append('Consider working longer or part-time in retirement')
            
            if shortfall_amount > 0:
                recommendations.append(f'Address retirement shortfall of ${shortfall_amount:,.0f}')
            
            if projected_savings > retirement_income_needed * 1.2:
                recommendations.append('Consider early retirement or increased spending')
    ...
    async def _identify_estate_tax_opportunities(self, estate_value: float, liquid_assets: float,
                                               illiquid_assets: float) -> List[str]:
    ...
    async def _generate_estate_recommendations(self, estate_value: float, estimated_taxes: float,
                                             will_status: str, tax_opportunities: List[str]) -> List[str]:
        ...
        try:
            ...
            if estimated_taxes > 0:
                recommendations.append(f'Plan for estate taxes of ${estimated_taxes:,.0f}')
    ...
    async def _calculate_life_insurance_needed(self, annual_income: float, dependents: int) -> float:
    ...
    async def _generate_insurance_recommendations(self, life_gap: float, disability_gap: float,
                                                long_term_care_gap: float) -> List[str]:
        """Generate insurance recommendations"""
        try:
            recommendations = []
            
            if life_gap > 0:
                recommendations.append(f'Consider ${life_gap:,.0f} in additional life insurance')
            
            if disability_gap > 0:
                recommendations.append(f'Consider ${disability_gap:,.0f} in disability insurance')
            
            if long_term_care_gap > 0:
                recommendations.append(f'Consider ${long_term_care_gap:,.0f} in long-term care insurance')
...
# Factory function
async def get_financial_planning_service(redis_client: redis.Redis, db_session: Session) -> FinancialPlanningService:
