...
class MachineLearningService:
    ...
    def __init__(self):
        ...
        self.model_configs = {
            ...
            "user_behavior": {"type": "linear_regression", "params": {}},
        }
    ...
    async def _train_price_prediction_model(self):
        ...
        try:
            ...
            logger.info(
                f"Price prediction model trained - MSE: {mse:.4f}, R²: {r2:.4f}"
            )
    ...
    async def _train_volume_prediction_model(self):
        ...
        try:
            ...
            logger.info(
                f"Volume prediction model trained - MSE: {mse:.4f}, R²: {r2:.4f}"
            )
    ...
    async def predict_market_price(
        self, market_id: int, horizon: str = "24h"
        ...
        try:
            ...
            if cache_key in self.prediction_cache:
                ...
                if (
                    datetime.utcnow() - cached_pred.timestamp
    ...
    async def generate_trading_recommendation(
        self, user_id: int, market_id: int
    ...
    def _generate_recommendation_reasoning(
        ...
        if recommendation_type == "buy":
            return (
                f"Market predicted to increase by {expected_return:.1f}% with {prediction.confidence:.1%} confidence. "
                f"Your risk tolerance ({user_profile.risk_tolerance:.1%}) supports this position."
            )
        elif recommendation_type == "sell":
            return (
                f"Market predicted to decrease by {abs(expected_return):.1f}% with {prediction.confidence:.1%} confidence. "
                f"Consider reducing exposure given the expected decline."
            )
        else:
            return (
                f"Expected return of {expected_return:.1f}% is below threshold for action. "
    ...
    async def _get_historical_market_data(self) -> List[Dict]:
        ...
        try:
            ...
            trades = (
                db.query(Trade)
                .filter(Trade.created_at >= thirty_days_ago)
            ...
            for trade in trades:
                ...
                market_data[market_id][hour]["avg_price"] = (
                    market_data[market_id][hour]["avg_price"]
                    * (len(market_data[market_id][hour]["trades"]) - 1)
                    + trade.price_per_share
                ) / len(market_data[market_id][hour]["trades"])
        ...
        for market_id, hourly_data in market_data.items():
            ...
            for i in range(1, len(hours)):
                ...
                feature_vector = [
                    ...
                    (current_hour - prev_hour).total_seconds()
                    / 3600,  # hours difference
                ]
    ...
    async def _get_user_trading_data(self, user_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            markets = (
                db.query(Market)
                .filter(Market.id.in_([t.market_id for t in trades]))
                .all()
            )
    ...
    def _calculate_prediction_confidence(self, market_id: int, horizon: str) -> float:
        ...
        horizon_multipliers = {"1h": 0.9, "24h": 0.7, "7d": 0.5}
    ...
    async def _get_market_features(self, market_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            avg_price = sum(t.price_per_share for t in recent_trades) / len(
                recent_trades
            )
            ...
            time_diff = (
                recent_trades[0].created_at - recent_trades[-1].created_at
            ).total_seconds() / 3600
    ...
    def _prepare_volume_prediction_data(
