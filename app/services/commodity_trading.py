...
class CommodityTradingService:
    ...
    def __init__(self, redis_client: redis_sync.Redis, db_session: Session):
        ...
        self.inventory_history: Dict[str, deque] = defaultdict(
    ...
    async def add_commodity_price(
        ...
        try:
            ...
            if len(self.commodity_prices[commodity_id]) > 1000:
                self.commodity_prices[commodity_id] = self.commodity_prices[
                    commodity_id
                ][-1000:]
            ...
            logger.info(
                f"Added {price_type} price for commodity {commodity_id}: {price} {currency}"
            )
            return price_data
    ...
    async def create_physical_delivery(
        self,
        contract_id: str,
        buyer_id: int,
        seller_id: int,
        ...
        try:
            delivery_id = (
                f"delivery_{contract_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    async def create_commodity_inventory(
        self,
        user_id: int,
        commodity_id: str,
        quantity: float,
        ...
        try:
            ...
            if facility.available_capacity < quantity:
                raise ValueError(
                    f"Insufficient storage capacity. Available: {facility.available_capacity}, Required: {quantity}"
                )
            ...
            current_value = (
                quantity * current_price if current_price else acquisition_cost
            )
    ...
    async def create_commodity_spread(
        self,
        commodity_id: str,
        spread_type: str,
        leg1_contract: str,
    ...
    async def calculate_carry_costs(
        self,
        commodity_id: str,
        spot_price: float,
        futures_price: float,
    ...
    async def get_storage_facilities(
        self, commodity_type: Optional[str] = None, location: Optional[str] = None
    ) -> List[Dict[str, Any]]:
                ...
                facilities.append(
                    {
                        ...
                        "utilization_rate": (
                            facility.total_capacity - facility.available_capacity
                        )
        ...
        try:
            ...
            if len(price_values) >= 2:
                ...
                price_change_percent = (
                    (price_change / price_values[0]) * 100
                    if price_values[0] != 0
                    else 0
                )
                ...
                sma_10 = (
                    np.mean(price_values[-10:]) if len(price_values) >= 10 else None
                )
                sma_20 = (
                    np.mean(price_values[-20:]) if len(price_values) >= 20 else None
                )
    ...
    async def _get_storage_capacity(self, commodity_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            utilization_rate = (
                (total_capacity - available_capacity) / total_capacity
                if total_capacity > 0
                else 0
            )
            ...
            try:
                ...
                for commodity in self.commodities.values():
                    if commodity.is_active:
                        ...
                        current_price = await self._get_current_commodity_price(
            ...
            try:
                ...
                for delivery in self.physical_deliveries.values():
                    if (
                        ...
                        and delivery.delivery_date <= current_time
                    ):
                        ...
                        logger.info(
                            f"Updated delivery {delivery.delivery_id} status to in_transit"
                        )
    ...
    async def _cache_commodity_price(self, price: CommodityPrice):
        ...
        try:
            cache_key = (
                f"commodity_price:{price.commodity_id}:{price.timestamp.isoformat()}"
            )
            await self.redis.setex(
                cache_key,
                3600,  # 1 hour TTL
