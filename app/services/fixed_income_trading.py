...
class FixedIncomeTradingService:
    ...
    async def create_bond_position(
        self, user_id: int, bond_id: str, quantity: float, entry_price: float
    ) -> BondPosition:
    ...
    async def get_yield_curve_analysis(
        self, curve_type: str, currency: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            curves = [
                c
                for c in self.yield_curves.values()
            ...
            yield_slope = (
                curve.yields[-1] - curve.yields[0] if len(curve.yields) > 1 else 0
            )

            # Calculate duration-weighted average yield
            durations = [self._tenor_to_years(tenor) for tenor in curve.tenors]
            duration_weighted_yield = (
                sum(y * d for y, d in zip(curve.yields, durations)) / sum(durations)
                if durations
                else 0
            )
            ...
            analysis = {
                ...
                "historical_comparison": await self._get_historical_curve_comparison(
                    curve_type, currency
                ),
                "last_updated": datetime.utcnow().isoformat(),
            }
        ...
        try:
            ...
            while last_coupon < datetime.utcnow() and last_coupon < bond.maturity_date:
                if bond.coupon_frequency == "annual":
                    ...
                    last_coupon = (
                        last_coupon.replace(year=last_coupon.year + 1)
                        if last_coupon.month >= 7
                        else last_coupon.replace(month=last_coupon.month + 6)
                    )
            ...
            days_in_period = (
                365
                if bond.coupon_frequency == "annual"
                else 182 if bond.coupon_frequency == "semi_annual" else 91
            )
    ...
    async def _get_historical_curve_comparison(
        self, curve_type: str, currency: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            yield_changes = [
                ...
                for current, previous in zip(
                    current_curve.yields, previous_curve.yields
                )
            ]
            ...
            return {
                ...
                "avg_change": (
                    sum(yield_changes) / len(yield_changes) if yield_changes else 0
                ),
            }
                ...
                for bond in self.bonds.values():
                    if bond.is_active:
                        ...
                        if current_price:
                            ...
                            new_clean_price = current_price.clean_price * (
                                1 + movement / 100
                            )
                            new_ytm = (
                            ...
                            await self.add_bond_price(
                                ...
                                new_clean_price,
                                new_ytm,
                                new_clean_price * 0.999,
                                new_clean_price * 1.001,
                                new_ytm + 0.05,
                                new_ytm - 0.05,
                                current_price.spread_to_treasury,
                                "simulated",
                            )
            ...
            try:
                for position in self.bond_positions.values():
                    ...
                    if current_price:
                        ...
                        position.duration_exposure = (
                            current_price.duration * position.quantity
                        )
                        ...
                        if bond:
                            position.accrued_interest = (
                                self._calculate_accrued_interest(bond)
                                * position.quantity
                            )
