...
class NotificationService:
    ...
    def notify_market_resolved(self, market_id: int, outcome: str):
        ...
        traders = self.db.query(Trade.user_id).filter(
    ...
    def notify_price_alert(self, user_id: int, market_id: int, old_price: float, new_price: float):
    ...
    def notify_dispute_created(self, dispute_id: int):
        ...
        dispute = self.db.query(MarketDispute).filter(MarketDispute.id == dispute_id).first()
        if not dispute:
            return
    ...
    def notify_system_announcement(self, title: str, message: str, user_ids: Optional[list] = None):
