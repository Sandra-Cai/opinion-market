...
from app.schemas.order import OrderCreate, OrderUpdate, OrderResponse, OrderListResponse, OrderBookResponse
...
def create_order(
    ...
    if order_data.side == OrderSide.BUY:
        required_balance = order_data.original_amount * (order_data.limit_price or market.current_price_a)
...
def get_order_book(
    ...
    ).order_by(desc(Order.limit_price)).limit(levels).all()
    ...
    sell_orders = db.query(Order).filter(
        ...
        Order.status.in_([OrderStatus.PENDING, OrderStatus.PARTIAL])
    ).order_by(Order.limit_price).limit(levels).all()
...
def _match_market_order(order: Order, db: Session):
    ...
    if order.side == OrderSide.BUY:
        ...
        ).order_by(Order.limit_price).all()
    else:
        ...
        matching_orders = db.query(Order).filter(
            ...
            Order.status.in_([OrderStatus.PENDING, OrderStatus.PARTIAL]),
            Order.outcome == order.outcome
        ).order_by(desc(Order.limit_price)).all()
