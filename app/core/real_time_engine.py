...
class RealTimeEngine:
    ...
    async def start(self):
        ...
        for stream_type in StreamType:
            processor_task = asyncio.create_task(self._process_stream(stream_type))
    ...
    async def unsubscribe(self, subscription_id: str):
        ...
        try:
            if subscription_id in self.subscriptions:
                ...
                logger.info(
                    f"User {subscription.user_id} unsubscribed from {subscription.stream_type.value}"
                )
    ...
    async def _generate_stream_data(
    ...
    def _cache_message(self, message: StreamMessage):
        ...
        self.data_cache[cache_key] = {"message": message, "cached_at": datetime.now()}
            ...
            for subscriber in filtered_subscribers:
                if subscriber.id in self.websocket_connections:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error sending to subscriber {subscriber.id}: {e}"
                        )
    ...
    async def get_stream_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cached_data": {
                stream_type.value: (
                    "available"
                    if f"{stream_type.value}_latest" in self.data_cache
                    else "none"
                )
                for stream_type in StreamType
            },
