...
async def train_ml_model(
    ...
    try:
        ...
        # Train the model
        success = await ml_model_manager.train_prediction_model(
...
async def make_prediction(
    ...
    try:
        if not input_data:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Input data is required"
            )
        ...
        if "error" in prediction:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=prediction["error"]
            )
...
async def optimize_trading_strategy(
    ...
    try:
        ...
        # Optimize strategy
        optimization_result = await ml_model_manager.optimize_trading_strategy(
            historical_data
        )
...
async def perform_cluster_analysis(
    ...
    try:
        ...
        # Perform clustering
        cluster_result = await ml_model_manager.cluster_market_segments(
            data, n_clusters
        )
...
async def get_model_info(
    model_name: str, current_user: Dict[str, Any] = Depends(get_current_user)
):
    ...
    try:
        ...
        return {"model_info": model_info, "timestamp": model_info["created_at"]}
...
async def get_ml_performance_analytics(
    ...
    try:
        ...
        ml_metrics = {
            ...
            "average_accuracy": (
                sum(m["metrics"]["accuracy"] for m in models) / len(models)
                if models
                else 0
            ),
            "total_predictions": metrics.get("counters", {}).get(
                "ml_predictions_total", 0
            ),
            "prediction_accuracy": metrics.get("gauges", {}).get(
...
def _generate_ml_recommendations(ml_metrics: Dict[str, Any]) -> List[str]:
    ...
    if ml_metrics["total_models"] == 0:
        recommendations.append(
            "No models trained - consider training prediction models"
        )

    if ml_metrics["average_accuracy"] < 0.7:
        recommendations.append(
            "Model accuracy is low - consider retraining with more data"
        )

    if ml_metrics["total_predictions"] == 0:
        recommendations.append(
            "No predictions made - start using models for market analysis"
        )

    if ml_metrics["prediction_accuracy"] < 0.8:
        recommendations.append(
            "Prediction accuracy needs improvement - review feature engineering"
        )
