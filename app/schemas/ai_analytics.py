...
class PredictionResponse(BaseModel):
    ...
    probability_a: float = Field(..., ge=0.0, le=1.0, description="Probability of outcome A")
    probability_b: float = Field(..., ge=0.0, le=1.0, description="Probability of outcome B")
    ...
    validity_duration: timedelta = Field(..., description="How long prediction is valid")
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "features_used": ["total_volume", "participant_count", "price_volatility"]
            }
        }
...
class MarketAnalyticsResponse(BaseModel):
    ...
    liquidity_score: float = Field(..., ge=0.0, le=1.0, description="Liquidity score (0-1)")
    ...
    prediction_accuracy: float = Field(..., ge=0.0, le=1.0, description="Historical prediction accuracy")
...
class UserAnalyticsResponse(BaseModel):
    ...
    preferred_categories: List[str] = Field(..., description="Preferred trading categories")
...
class MarketInsightResponse(BaseModel):
    ...
    analytics: Optional[Dict[str, Any]] = Field(None, description="Market analytics data")
...
class PortfolioInsightResponse(BaseModel):
    ...
    performance_analysis: Dict[str, str] = Field(..., description="Performance analysis")
