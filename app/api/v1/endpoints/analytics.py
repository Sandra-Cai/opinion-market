...
def get_trending_analytics(
    ...
    trending_markets = db.query(Market).filter(
        Market.status == 'open'
    ).order_by(Market.trending_score.desc()).limit(limit).all()
...
def get_sentiment_analysis(
    ...
    query = db.query(
        Trade.market_id,
        func.sum(func.case([(Trade.trade_type == 'buy', Trade.total_value)], else_=0)).label('buy_volume'),
        func.sum(func.case([(Trade.trade_type == 'sell', Trade.total_value)], else_=0)).label('sell_volume'),
        func.count(func.case([(Trade.trade_type == 'buy', 1)], else_=None)).label('buy_trades'),
        func.count(func.case([(Trade.trade_type == 'sell', 1)], else_=None)).label('sell_trades')
    )
