...
class MarketDataFeed:
    ...
    async def _collect_market_data(self):
        ...
        try:
            ...
            for market in markets:
                ...
                volume_24h = db.query(Trade).filter(
                    Trade.market_id == market.id,
                    Trade.created_at >= yesterday
                ).with_entities(
                    db.func.sum(Trade.total_value)
                ).scalar() or 0.0
                
                # Get unique traders in last 24h
                unique_traders = db.query(Trade).filter(
                    Trade.market_id == market.id,
                    Trade.created_at >= yesterday
                ).with_entities(
                    db.func.count(db.func.distinct(Trade.user_id))
                ).scalar() or 0
                ...
                # Calculate spread
                spread = abs(current_price_a - current_price_b) / max(current_price_a, current_price_b)
        ...
        try:
            ...
            if abs(data_point.price_change_1h) > self.alert_thresholds['price_spike']:
                alert = MarketAlert(
                    ...
                    severity='high' if abs(data_point.price_change_1h) > 0.2 else 'medium',
                    timestamp=datetime.utcnow(),
            ...
            # Volume surge alert
            if data_point.volume_24h > self._get_average_volume(data_point.market_id) * self.alert_thresholds['volume_surge']:
                alert = MarketAlert(
                    market_id=data_point.market_id,
            ...
            if total_liquidity < self._get_average_liquidity(data_point.market_id) * (1 - self.alert_thresholds['liquidity_drop']):
                alert = MarketAlert(
                    market_id=data_point.market_id,
            ...
            if data_point.spread > self._get_average_spread(data_point.market_id) * (1 + self.alert_thresholds['spread_widening']):
                alert = MarketAlert(
                    market_id=data_point.market_id,
    ...
    async def _alert_monitoring_loop(self):
        ...
        while True:
            try:
                ...
                critical_alerts = [a for a in self.market_alerts if a.severity == 'critical']
    ...
    def _calculate_volatility(self, market_id: int) -> float:
        ...
        try:
            ...
            for i in range(1, len(trades)):
                change = (trades[i].price_per_share - trades[i-1].price_per_share) / trades[i-1].price_per_share
    ...
    def _get_average_volume(self, market_id: int) -> float:
        ...
        try:
            ...
            total_volume = db.query(Trade).filter(
                Trade.market_id == market_id,
    ...
    def _get_average_liquidity(self, market_id: int) -> float:
        ...
        try:
            ...
            market = db.query(Market).filter(Market.id == market_id).first()
            if market:
                return (market.liquidity_pool_a or 0.0) + (market.liquidity_pool_b or 0.0)
            return 1000.0  # Default value
    ...
    def _get_average_spread(self, market_id: int) -> float:
        ...
        try:
            ...
            market = db.query(Market).filter(Market.id == market_id).first()
            if market:
                return abs(market.current_price_a - market.current_price_b) / max(market.current_price_a, market.current_price_b)
    ...
    async def _consider_trading_pause(self, alert: MarketAlert):
        ...
        logger.warning(f"Considering trading pause for market {alert.market_id}: {alert.message}")
    ...
    async def get_market_alerts(self, market_id: Optional[int] = None, severity: Optional[str] = None) -> List[MarketAlert]:
