...
async def create_blockchain_wallet(
    network: str, current_user: Dict[str, Any] = Depends(get_current_user)
):
    ...
    try:
        ...
        wallet_info = await blockchain_manager.create_wallet(
            blockchain_network, current_user["user_id"]
        )
...
async def send_blockchain_transaction(
    ...
    try:
        ...
        transaction = await blockchain_manager.send_transaction(
            from_address, to_address, amount, token, blockchain_network
        )
...
async def get_transaction_status(
    tx_hash: str, current_user: Dict[str, Any] = Depends(get_current_user)
):
...
async def deploy_smart_contract(
    ...
    try:
        ...
        if not abi or not isinstance(abi, dict):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Valid ABI is required"
            )

        # Deploy contract
        contract = await blockchain_manager.deploy_smart_contract(
            contract_name, abi, blockchain_network, current_user["user_id"]
        )
...
async def call_contract_function(
    ...
    try:
        ...
        result = await blockchain_manager.call_contract_function(
            contract_address, function_name, parameters, current_user["user_id"]
        )
...
async def get_deployed_contracts(
    ...
    try:
        ...
        return {"contracts": contracts, "total_contracts": len(contracts)}
...
async def estimate_gas_cost(
    ...
    try:
        ...
        gas_estimate = await blockchain_manager.estimate_gas(
            from_address, to_address, amount, blockchain_network
        )
...
async def get_blockchain_analytics(
    ...
    try:
        ...
        pending_transactions = len(
            [
                tx
                for tx in blockchain_manager.transactions.values()
        ...
        return {
            "analytics": {
                ...
                "success_rate": (
                    (confirmed_transactions / total_transactions * 100)
