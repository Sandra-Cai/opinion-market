...
class OrderManagementSystem:
    ...
    async def create_order(self, user_id: int, account_id: str, symbol: str,
                          order_type: OrderType, side: OrderSide, quantity: float,
                          price: Optional[float] = None, stop_price: Optional[float] = None,
                          time_in_force: TimeInForce = TimeInForce.DAY,
                          client_order_id: Optional[str] = None,
                          algo_type: Optional[str] = None,
                          algo_parameters: Optional[Dict[str, Any]] = None) -> Order:
        """Create a new order"""
        try:
            # Generate order ID
    ...
    async def cancel_order(self, order_id: str, user_id: int) -> bool:
        ...
        try:
            ...
            if order.status in [OrderStatus.FILLED, OrderStatus.CANCELLED, OrderStatus.REJECTED]:
                logger.warning(f"Order {order_id} cannot be cancelled in status {order.status}")
                return False
    ...
    async def modify_order(self, order_id: str, user_id: int, new_quantity: Optional[float] = None,
        ...
        try:
            ...
            if order.status in [OrderStatus.FILLED, OrderStatus.CANCELLED, OrderStatus.REJECTED]:
                logger.warning(f"Order {order_id} cannot be modified in status {order.status}")
                return False
            
            # Check user authorization
            if order.user_id != user_id:
                logger.warning(f"User {user_id} not authorized to modify order {order_id}")
                return False
    ...
    async def get_orders(self, user_id: int, account_id: Optional[str] = None,
    ...
    async def get_execution_reports(self, order_id: str, user_id: int) -> List[ExecutionReport]:
    ...
    async def _check_position_limits(self, order: Order) -> bool:
        ...
        try:
            ...
            current_position = await self._get_current_position(order.user_id, order.account_id, order.symbol)
    ...
    async def _get_current_position(self, user_id: int, account_id: str, symbol: str) -> float:
        ...
        try:
            ...
            for order in self.orders.values():
                if (order.user_id == user_id and 
                    order.account_id == account_id and 
    ...
    async def _generate_execution_report(self, order: Order, execution_type: str, text: Optional[str] = None):
        """Generate execution report"""
        try:
        ...
        while True:
            try:
                ...
                for user_id in set(order.user_id for order in self.orders.values()):
                    for account_id in set(order.account_id for order in self.orders.values() if order.user_id == user_id):
                        for symbol in set(order.symbol for order in self.orders.values() if order.user_id == user_id and order.account_id == account_id):
                            position = await self._get_current_position(user_id, account_id, symbol)
                            limits = self.position_limits.get(account_id, {})
                            max_position = limits.get(symbol, float('inf'))
                            
                            if abs(position) > max_position:
                                logger.warning(f"Position limit exceeded for {user_id}/{account_id}/{symbol}")
    ...
    async def _simulate_order_fill(self, order: Order):
        ...
        try:
            ...
            fill_price = order.price or (self.market_data.get(order.symbol, MarketData(
                symbol=order.symbol, bid_price=100, ask_price=100.01, bid_size=1000,
                ask_size=1000, last_price=100, volume=10000, high_price=101,
                low_price=99, open_price=100, timestamp=datetime.utcnow()
            )).last_price)
            ...
            order.average_price = order.total_filled_value / order.filled_quantity if order.filled_quantity > 0 else 0
            ...
            # Generate execution report
            await self._generate_execution_report(order, 'partial_fill' if order.remaining_quantity > 0 else 'fill', 
            ...
            logger.info(f"Simulated fill for order {order.order_id}: {fill_quantity} @ {fill_price}")
        ...
        while True:
            try:
                ...
                for order_id, order in self.orders.items():
                    if order.created_at < cutoff_date and order.status in [OrderStatus.FILLED, OrderStatus.CANCELLED, OrderStatus.REJECTED]:
...
# Factory function
async def get_order_management_system(redis_client: redis.Redis, db_session: Session) -> OrderManagementSystem:
