...
class AlternativeInvestmentsService:
    ...
    def __init__(self, redis_client: redis.Redis, db_session: Session):
        ...
        self.performance_metrics: Dict[str, List[PerformanceMetrics]] = defaultdict(
            list
        )
    ...
    async def create_commitment(
        self, user_id: int, investment_id: str, commitment_amount: float
        ...
        try:
            ...
            if commitment_amount < investment.minimum_investment:
                raise ValueError(
                    f"Commitment amount below minimum: {investment.minimum_investment}"
                )
    ...
    async def calculate_performance_metrics(
        self, investment_id: str, period: str = "total"
    ) -> PerformanceMetrics:
        ...
        try:
            ...
            # Calculate metrics
            total_invested = sum(
                call.paid_amount for call in calls if call.status == "paid"
            )
            ...
            # Net multiple
            net_multiple = (
                (total_distributed + current_nav) / total_invested
                if total_invested > 0
                else 1.0
            )
            ...
            benchmark_return = await self._get_benchmark_return(
                investment.investment_type, years
            )
            ...
            information_ratio = (
                jensen_alpha / tracking_error if tracking_error > 0 else 0.0
            )
    ...
    async def create_valuation(
        ...
        try:
            ...
            realized_gain_loss = (
                sum(dist.distribution_amount for dist in distributions) - cost_basis
            )
            unrealized_gain_loss = fair_value - (
                cost_basis - sum(dist.distribution_amount for dist in distributions)
            )
    ...
    async def get_portfolio_analytics(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            total_nav = sum(
                self.investments[c.investment_id].net_asset_value
            ...
            portfolio_multiple = (
                (total_distributed + total_nav) / total_called
                if total_called > 0
                else 1.0
            )
                    ...
                    for user_id, commitments in self.commitments.items():
                        for commitment in commitments:
                            if commitment.investment_id == call.investment_id:
                                commitment.called_amount += call.call_amount * (
    ...
    async def _get_benchmark_return(
        self, investment_type: AlternativeInvestmentType, years: float
    ) -> float:
    ...
    async def _calculate_portfolio_irr(
        self, commitments: List[InvestmentCommitment]
    ) -> float:
            ...
            for commitment in commitments:
                ...
                for dist in distributions:
                    all_cash_flows.append(
                        ...
                        * (commitment.commitment_amount / investment.committed_capital)
                    )
    ...
    async def _calculate_portfolio_risk_metrics(
        ...
        try:
            ...
            # Concentration risk
            max_commitment = (
                max(c.commitment_amount for c in commitments) if commitments else 0
            )
            concentration_risk = (
                max_commitment / total_committed if total_committed > 0 else 0
            )

            # Diversification
            unique_types = len(
                set(
                    self.investments[c.investment_id].investment_type
            ...
            # Liquidity risk
            illiquid_commitments = sum(
                c.commitment_amount
                for c in commitments
                if self.investments[c.investment_id].liquidity_profile
                in [LiquidityProfile.ILLIQUID, LiquidityProfile.VERY_ILLIQUID]
            )
            liquidity_risk = (
                illiquid_commitments / total_committed if total_committed > 0 else 0
            )
    ...
    async def _calculate_performance_attribution(
        ...
        try:
            ...
            for commitment in commitments:
                ...
                weight = (
                    commitment.commitment_amount / total_committed
