...
class OptionsStrategy:
    ...
    strategy_id: str
    strategy_name: str
    strategy_type: str  # 'spread', 'straddle', 'strangle', 'butterfly', 'iron_condor', etc.
...
class OptionsTradingService:
    ...
    async def create_option_contract(self, underlying_asset: str, option_type: str, strike_price: float,
                                   expiration_date: datetime, current_price: float, underlying_price: float,
                                   volatility: float, dividend_yield: float = 0.0) -> OptionContract:
    ...
    async def calculate_black_scholes_price(self, contract: OptionContract) -> float:
        ...
        try:
            ...
            if T <= 0:
                return max(0, S - K) if contract.option_type == 'call' else max(0, K - S)
    ...
    async def calculate_greeks(self, contract: OptionContract) -> OptionGreeks:
        ...
        try:
            ...
            if T <= 0:
                ...
                volga = S * np.exp(-q * T) * self._normal_pdf(d1) * np.sqrt(T) * d1 * d2 / sigma
                charm = (q * np.exp(-q * T) * self._normal_cdf(d1) 
                        - np.exp(-q * T) * self._normal_pdf(d1) * (2 * (r - q) * T - d2 * sigma * np.sqrt(T)) / (2 * T * sigma * np.sqrt(T)))
    ...
    async def calculate_implied_volatility(self, contract: OptionContract, market_price: float) -> float:
    ...
    async def create_volatility_surface(self, underlying_asset: str, expiration_dates: List[datetime],
                                      strike_prices: List[float], implied_volatilities: List[List[float]]) -> VolatilitySurface:
    ...
    async def interpolate_volatility(self, underlying_asset: str, strike: float, expiration: datetime) -> float:
        ...
        try:
            ...
            exp_idx = min(range(len(surface.expiration_dates)), 
                         key=lambda i: abs((surface.expiration_dates[i] - expiration).days))
            strike_idx = min(range(len(surface.strike_prices)), 
    ...
    async def create_options_strategy(self, strategy_name: str, strategy_type: str, legs: List[Dict[str, Any]]) -> OptionsStrategy:
        """Create an options trading strategy"""
        try:
            strategy_id = f"strategy_{strategy_name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            
            # Calculate strategy metrics
            max_profit, max_loss, breakeven_points = await self._calculate_strategy_metrics(legs)
    ...
    async def create_options_position(self, user_id: int, contract_id: str, quantity: int,
                                    entry_price: float, position_type: str) -> OptionsPosition:
        ...
        try:
            ...
            unrealized_pnl = (current_price - entry_price) * quantity if position_type == 'long' else (entry_price - current_price) * quantity
    ...
    async def get_options_chain(self, underlying_asset: str, expiration_date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for contract in self.option_contracts.values():
                if contract.underlying_asset == underlying_asset:
                    if expiration_date is None or contract.expiration_date == expiration_date:
    ...
    async def calculate_portfolio_greeks(self, user_id: int) -> Dict[str, float]:
        ...
        try:
            user_positions = [
                pos for pos in self.options_positions.values()
    ...
    async def _calculate_option_pricing(self, contract: OptionContract):
        ...
        try:
            ...
            intrinsic_value = max(0, contract.underlying_price - contract.strike_price) if contract.option_type == 'call' else max(0, contract.strike_price - contract.underlying_price)
    ...
    async def _calculate_strategy_metrics(self, legs: List[Dict[str, Any]]) -> Tuple[float, float, List[float]]:
        """Calculate strategy metrics"""
        try:
    ...
    async def _calculate_probability_of_profit(self, legs: List[Dict[str, Any]]) -> float:
            ...
            try:
                for position in self.options_positions.values():
                    ...
                    if contract:
                        ...
                        position.unrealized_pnl = (current_price - position.entry_price) * position.quantity if position.position_type == 'long' else (position.entry_price - current_price) * position.quantity
    ...
    async def _cache_volatility_surface(self, surface: VolatilitySurface):
        ...
        try:
            ...
            await self.redis.setex(
                ...
                3600,  # 1 hour TTL
                json.dumps({
                    'expiration_dates': [d.isoformat() for d in surface.expiration_dates],
...
# Factory function
async def get_options_trading_service(redis_client: redis.Redis, db_session: Session) -> OptionsTradingService:
