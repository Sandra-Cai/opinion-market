...
def get_achievement_details(
    ...
    achievement = next(
        (a for a in achievements["achievements"] if a["id"] == achievement_id),
        None
    )
        ...
        try:
            ...
            if "error" not in achievements:
                user_achievements.append({
                    ...
                    "completion_percentage": (achievements["total_unlocked"] / achievements["total_achievements"]) * 100 if achievements["total_achievements"] > 0 else 0
                })
        except:
            continue
...
def get_rewards_stats(
    ...
    return {
        ...
        "completion_percentage": (len(unlocked_achievements) / len(achievements["achievements"])) * 100 if achievements["achievements"] else 0,
        "rewards_earned": {
        ...
        "recent_achievements": unlocked_achievements[-5:] if len(unlocked_achievements) > 5 else unlocked_achievements,
...
def get_user_milestones(
    ...
    for milestone in volume_milestones:
        progress = min(100, (current_user.total_volume / milestone) * 100) if milestone > 0 else 0
    ...
    for milestone in profit_milestones:
        progress = min(100, (current_user.total_profit / milestone) * 100) if milestone > 0 else 0
    ...
    for milestone in trade_milestones:
        progress = min(100, (current_user.total_trades / milestone) * 100) if milestone > 0 else 0
...
def get_user_streaks(
    ...
    login_streak = 1 if current_user.last_login and current_user.last_login.date() == datetime.utcnow().date() else 0
    ...
    return {
        ...
        "trading_streak": {
            ...
            "longest": len(set(t.created_at.date() for t in recent_trades)),  # Simplified
...
def get_recent_rewards(
    ...
    recent_achievements = [a for a in achievements["achievements"] if a["unlocked"]][-limit:]
