...
class AIPredictionService:
    ...
    def __init__(self, redis_client: redis.Redis, db_session: Session):
        ...
        self.feature_columns = [
            'total_volume', 'participant_count', 'days_remaining',
            'price_volatility', 'volume_trend', 'social_sentiment',
            'news_sentiment', 'historical_accuracy', 'category_risk',
            'liquidity_score', 'momentum_indicator'
        ]
    ...
    async def _train_models(self):
        ...
        # Split data
        X_train, X_test, y_class_train, y_class_test, y_conf_train, y_conf_test = train_test_split(
            X, y_class, y_conf, test_size=0.2, random_state=42
        )
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(y_class_test, class_pred, average='weighted')
        
        logger.info(f"Model training completed - Accuracy: {accuracy:.3f}, F1: {f1:.3f}")
    ...
    async def predict_market_outcome(self, market_id: int) -> Optional[PredictionResult]:
        ...
        try:
            ...
            outcome_probabilities = self.models['classifier'].predict_proba(scaled_features)[0]
    ...
    async def _extract_market_features(self, market_id: int) -> Optional[MarketFeatures]:
        ...
        try:
            ...
            category_risk = self._calculate_category_risk(market_data.get('category', ''))
    ...
    def _generate_recommendation(self, outcome: str, confidence: float, features: MarketFeatures) -> str:
...
# Factory function
async def get_ai_prediction_service(redis_client: redis.Redis, db_session: Session) -> AIPredictionService:
